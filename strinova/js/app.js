...
let onlineRoomDeletionRef = null;
...
let onlineRemoteRoulettePlaybackId = null;
let onlineRemoteMapPlaybackId = null;
...
const state = {
  ...
  intro: {
    ...
    selectedSetIndex: -1,
    selectedSet: null,
    silentPrecacheStarted: false,
    silentPrecachePromise: null,
    alternativesPrecacheStarted: false,
    alternativesPrecachePromise: null,
    menuVideoReady: false,
    menuAudioReady: false,
    selectedPairStatus: null,
    resolvedSetId: null,
    resolvedVideoSource: "",
    resolvedAudioSource: "",
    resolvedSourceKind: "",
...
function sanitizeStoredSettings(rawSettings = {}) {
  ...
  sanitized.introRandomEnabled = typeof rawSettings.introRandomEnabled === "boolean" ? rawSettings.introRandomEnabled : defaults.introRandomEnabled;
  sanitized.introManualSetId = Math.round(clampNumber(rawSettings.introManualSetId, 1, 5, defaults.introManualSetId));
...
const introRandomToggle = $("#intro-random-toggle");
const introManualSelect = $("#intro-manual-select");
const introSelectionStatus = $("#intro-selection-status");
...
function updateOnlineStaticTexts() {
  ...
  updateOnlineServiceNoticeLanguage();
}
...
function applyLanguage(lang = currentLanguage(), options = {}) {
  ...
  setText('.intro-settings-heading span','intro_settings_small');
  setText('.intro-settings-heading strong','intro_settings_title');
  setText('.intro-random-row .subconfig-copy span','intro_random_selection');
  setText('.intro-random-row .subconfig-copy small','intro_random_selection_desc');
  setText('.intro-manual-copy span','intro_manual_selection');
  setText('.intro-manual-copy small','intro_manual_selection_desc');
  updateIntroSelectionSettingsUI();
...
function serializeCharacterLayoutConfig() {
  ...
  return `window.CHARACTER_LAYOUT_CONFIG = ${JSON.stringify(payload, null, 2)};\n`;
}
...
async function runResourcePreloadQueue(groups, onProgress, concurrency = RESOURCE_PRELOAD_CONCURRENCY) {
...
function scheduleSecondaryResourcePreload() {
  ...
  window.setTimeout(() => {
    if (typeof window.requestIdleCallback === "function") {
      window.requestIdleCallback(queue, { timeout: 3500 });
    } else {
      queue();
    }
  }, 1400);
...
function scheduleGeneralSilentPrecache() {
  if (state.resourcePreload.started || state.resourcePreload.completed) return state.resourcePreload.promise || Promise.resolve();
  const queue = () => preloadCriticalResources({
    showOverlay: false,
    concurrency: RESOURCE_PRELOAD_BACKGROUND_CONCURRENCY,
    scheduleSecondary: false,
  });
  if (typeof window.requestIdleCallback === "function") {
    return new Promise(resolve => {
      window.requestIdleCallback(() => queue().finally(resolve), { timeout: 1500 });
    });
  }
  return new Promise(resolve => window.setTimeout(() => queue().finally(resolve), 320));
}

function preloadCriticalResources({ showOverlay = true, concurrency = RESOURCE_PRELOAD_CONCURRENCY, scheduleSecondary = true } = {}) {
  if (state.resourcePreload.completed) {
    if (scheduleSecondary) scheduleSecondaryResourcePreload();
...
function setupBackgroundVideo() {
  ...
  const scheduleRecovery = (delayMs = 2400, hard = false) => {
    clearScheduledRecovery();
    recoveryTimerId = window.setTimeout(() => {
      recoveryTimerId = null;
      if (document.hidden) return;
      const lastUpdateAt = Number(video.dataset.lastTimeUpdateAt || 0);
      const updateIsStale = lastUpdateAt > 0 && Date.now() - lastUpdateAt > 6500;
      const needsRecovery = hard || video.paused || video.readyState < 2 || updateIsStale;
      if (needsRecovery) recoverFrozenVideo(hard);
    }, delayMs);
  };
  ...
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) scheduleRecovery(450, false);
  });
  window.addEventListener("focus", () => scheduleRecovery(450, false));
  window.addEventListener("pageshow", () => scheduleRecovery(450, false));
  ...
  if (video.dataset.watchdog !== "true") {
    ...
    window.setInterval(() => {
      ...
      const lastUpdateAt = Number(video.dataset.lastTimeUpdateAt || 0);
      const timeUpdateIsOld = lastUpdateAt > 0 && Date.now() - lastUpdateAt > 6500;
      const seemsFrozen = !video.paused && video.readyState >= 2 && Math.abs(current - previous) < 0.02 && timeUpdateIsOld;
      ...
      const nextStallTicks = (seemsFrozen || video.readyState < 2) ? stallTicks + 1 : 0;
      video.dataset.stallTicks = String(nextStallTicks);
      if (nextStallTicks >= 3) {
        video.dataset.stallTicks = "0";
        scheduleRecovery(250, Boolean(video.error));
      }
    }, 2000);
  }
...
const DRAFT_ACTION_ANIMATION_PROFILE = Object.freeze({
  ban: Object.freeze({ manual: 880, automatic: 760 }),
  pick: Object.freeze({ manual: 820, automatic: 720 }),
});

function draftAnimationScale() {
  return clampNumber(state.settings.animationDuration, 0.6, 1.8, 1.6);
}

function confirmedActionAnimationDuration(type, isAuto = false) {
  const profile = DRAFT_ACTION_ANIMATION_PROFILE[type] || DRAFT_ACTION_ANIMATION_PROFILE.pick;
  const base = isAuto ? profile.automatic : profile.manual;
  return Math.max(360, Math.round(base * draftAnimationScale()));
}

function syncDraftAnimationTiming() {
  const rootStyle = document.documentElement.style;
  rootStyle.setProperty("--rpmods-ban-confirm-duration", `${confirmedActionAnimationDuration("ban")}ms`);
  rootStyle.setProperty("--rpmods-pick-confirm-duration", `${confirmedActionAnimationDuration("pick")}ms`);
  rootStyle.setProperty("--rpmods-slot-confirm-duration", `${confirmedActionAnimationDuration("pick")}ms`);
}

function resetIntroSelectionRuntime() {
  state.intro.selectedSet = null;
  state.intro.selectedSetIndex = -1;
  state.intro.resolvedSetId = null;
  state.intro.resolvedVideoSource = "";
  state.intro.resolvedAudioSource = "";
  state.intro.resolvedSourceKind = "";
}

function introMenuSetName(setId) {
  const numericId = Math.round(clampNumber(setId, 1, INTRO_MENU_SETS.length, 1));
  return INTRO_MENU_SETS.find(item => item.id === numericId)?.name || `INTRO ${String(numericId).padStart(2, "0")}`;
}

function populateIntroManualSelect() {
  if (!introManualSelect) return;
  const currentValue = String(Math.round(clampNumber(introManualSelect.value || state.settings.introManualSetId, 1, INTRO_MENU_SETS.length, 1)));
  const fragment = document.createDocumentFragment();
  INTRO_MENU_SETS.forEach(item => {
    const option = document.createElement("option");
  ...
  introManualSelect.replaceChildren(fragment);
  introManualSelect.value = currentValue;
}

function updateIntroSelectionSettingsUI() {
  const randomEnabled = state.settings.introRandomEnabled !== false;
  const manualId = Math.round(clampNumber(state.settings.introManualSetId, 1, 5, 1));
  if (introRandomToggle) introRandomToggle.checked = randomEnabled;
  if (introManualSelect) {
    introManualSelect.value = String(manualId);
    introManualSelect.disabled = randomEnabled;
    introManualSelect.setAttribute("aria-disabled", String(randomEnabled));
  }
  const panel = document.getElementById("intro-settings-panel");
  panel?.classList.toggle("is-random", randomEnabled);
  panel?.classList.toggle("is-manual", !randomEnabled);
  if (introSelectionStatus) {
    introSelectionStatus.textContent = randomEnabled
      ? t("intro_mode_random_status")
      : t("intro_mode_manual_status", { name: introMenuSetName(manualId) });
  }
}

function setupIntroSelectionControls() {
  populateIntroManualSelect();
  updateIntroSelectionSettingsUI();
  if (introRandomToggle) {
    introRandomToggle.addEventListener("change", () => {
      state.settings.introRandomEnabled = introRandomToggle.checked;
      resetIntroSelectionRuntime();
      updateIntroSelectionSettingsUI();
      scheduleSettingsSave();
    });
  }
  if (introManualSelect) {
    introManualSelect.addEventListener("change", () => {
      state.settings.introManualSetId = Math.round(clampNumber(introManualSelect.value, 1, 5, 1));
      resetIntroSelectionRuntime();
      updateIntroSelectionSettingsUI();
      scheduleSettingsSave();
    });
...
const INTRO_MENU_SETS = [
  {
    id: 1,
    name: "Strinova Season 9 (26sp3)",
    video: "media/intro-menu/intro-01/video.mp4",
    audio: "media/intro-menu/intro-01/music.mp3",
    legacyVideo: "video/introV/Intro_menu.mp4",
    legacyAudio: "audio/music_intro.mp3",
  },
  {
    id: 2,
    name: "Strinova Season 6 (Michelle Skin)",
    video: "media/intro-menu/intro-02/video.mp4",
    audio: "media/intro-menu/intro-02/music.mp3",
    legacyVideo: "video/introV/Intro_menu_2.mp4",
    legacyAudio: "audio/music_intro_2.mp3",
  },
  {
    id: 3,
    name: "Strinova Season 7 (p2)",
    video: "media/intro-menu/intro-03/video.mp4",
    audio: "media/intro-menu/intro-03/music.mp3",
    legacyVideo: "video/introV/Intro_menu_3.mp4",
    legacyAudio: "audio/music_intro_3.mp3",
  },
  {
    id: 4,
    name: "Strinova Season 7",
    video: "media/intro-menu/intro-04/video.mp4",
    audio: "media/intro-menu/intro-04/music.mp3",
    legacyVideo: "video/introV/Intro_menu_4.mp4",
    legacyAudio: "audio/music_intro_4.mp3",
  },
  {
    id: 5,
    name: "Kanami [Be Shining] - Strinova",
    video: "media/intro-menu/intro-05/video.mp4",
    audio: "media/intro-menu/intro-05/music.mp3",
    legacyVideo: "video/introV/Intro_menu_5.mp4",
    legacyAudio: "audio/music_intro_5.mp3",
  },
];

const INTRO_LAST_SET_STORAGE_KEY = "rpmods_last_intro_set_v1";
const INTRO_PAIR_PRELOAD_TIMEOUT_MS = 9000;
...
function chooseIntroMenuSet() {
  if (state.intro.selectedSet) return state.intro.selectedSet;

  const randomEnabled = state.settings.introRandomEnabled !== false;
  let selected = null;

  if (!randomEnabled) {
    const manualId = Math.round(clampNumber(state.settings.introManualSetId, 1, INTRO_MENU_SETS.length, 1));
    selected = INTRO_MENU_SETS.find(item => item.id === manualId) || INTRO_MENU_SETS[0];
  } else {
    let previousId = 0;
    try { previousId = Number(window.localStorage?.getItem(INTRO_LAST_SET_STORAGE_KEY) || 0); } catch (_) {}
    const candidates = INTRO_MENU_SETS.filter(item => item.id !== previousId);
    const pool = candidates.length ? candidates : INTRO_MENU_SETS;
    selected = pool[Math.floor(Math.random() * pool.length)] || INTRO_MENU_SETS[0];
  }

  state.intro.selectedSet = selected;
  state.intro.selectedSetIndex = Math.max(0, INTRO_MENU_SETS.indexOf(selected));
  try { window.localStorage?.setItem(INTRO_LAST_SET_STORAGE_KEY, String(selected.id)); } catch (_) {}
  return selected;
}

function currentIntroMenuSet() {
  return state.intro.selectedSet || chooseIntroMenuSet();
}
...
function createIntroOverlay() {
  ...
  const video = document.createElement("video");
  video.className = "intro-video intro-primary-video";

  const menuVideo = document.createElement("video");
  menuVideo.className = "intro-video intro-menu-video";
...
function introVideoElement() {
  ...
  return overlay.querySelector(".intro-primary-video") || overlay.querySelector(".intro-video");
}

function introMenuVideoElement() {
  const overlay = createIntroOverlay();
  return overlay.querySelector(".intro-menu-video");
}

function absoluteMediaUrl(source) {
  try { return new URL(String(source || ""), document.baseURI).href; }
  catch (_) { return String(source || ""); }
}

function mediaElementUsesSource(element, source) {
  if (!element || !source) return false;
  const active = element.currentSrc || element.src || "";
  return absoluteMediaUrl(active) === absoluteMediaUrl(source);
}

function inspectMediaSource(element, timeoutMs = INTRO_PAIR_PRELOAD_TIMEOUT_MS) {
  return new Promise(resolve => {
    if (!element) {
      resolve({ valid: false, ready: false, reason: "missing-element" });
      return;
    }
    if (element.error) {
      resolve({ valid: false, ready: false, reason: "media-error" });
      return;
    }
    if (element.readyState >= HTMLMediaElement.HAVE_METADATA) {
      resolve({ valid: true, ready: element.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA, reason: "metadata" });
      return;
    }
    ...
    const onMetadata = () => finish({ valid: true, ready: element.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA, reason: "metadata" });
    const onReady = () => finish({ valid: true, ready: true, reason: "ready" });
    const onError = () => finish({ valid: false, ready: false, reason: "media-error" });
    const timer = window.setTimeout(() => {
      if (element.error) {
        finish({ valid: false, ready: false, reason: "media-error" });
      } else {
        finish({ valid: null, ready: element.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA, reason: "still-loading" });
      }
    }, timeoutMs);

    element.addEventListener("loadedmetadata", onMetadata, { once: true });
    element.addEventListener("durationchange", onMetadata, { once: true });
...
function introSetCandidatePairs(set) {
  if (!set) return [];
  const candidates = [
    { video: set.video, audio: set.audio, source: "dedicated" },
    { video: set.legacyVideo, audio: set.legacyAudio, source: "legacy" },
  ];
  const seen = new Set();
  return candidates.filter(pair => {
    if (!pair.video || !pair.audio) return false;
    const key = `${absoluteMediaUrl(pair.video)}|${absoluteMediaUrl(pair.audio)}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });
...
function resolvedIntroPair(set = currentIntroMenuSet()) {
  if (
    state.intro.resolvedSetId === set?.id &&
    state.intro.resolvedVideoSource &&
    state.intro.resolvedAudioSource
  ) {
    return {
      video: state.intro.resolvedVideoSource,
      audio: state.intro.resolvedAudioSource,
      source: state.intro.resolvedSourceKind || "dedicated",
    };
  }
  return introSetCandidatePairs(set)[0] || { video: set?.video || "", audio: set?.audio || "", source: "dedicated" };
}

function rememberResolvedIntroPair(set, pair) {
  state.intro.resolvedSetId = set?.id ?? null;
  state.intro.resolvedVideoSource = pair?.video || "";
  state.intro.resolvedAudioSource = pair?.audio || "";
  state.intro.resolvedSourceKind = pair?.source || "dedicated";
}

async function loadSelectedIntroPair(set = currentIntroMenuSet(), preferredPair = null) {
  const menuVideo = introMenuVideoElement();
  if (!menuVideo) return { valid: false, video: null, audio: null };

  const baseCandidates = introSetCandidatePairs(set);
  const candidates = preferredPair
    ? [preferredPair, ...baseCandidates.filter(pair =>
        absoluteMediaUrl(pair.video) !== absoluteMediaUrl(preferredPair.video) ||
        absoluteMediaUrl(pair.audio) !== absoluteMediaUrl(preferredPair.audio)
      )]
    : baseCandidates;
  let lastResult = { valid: false, video: null, audio: null };
  ...
  for (const pair of candidates) {
    try {
      ...
      menuVideo.muted = true;
      menuVideo.volume = 0;
      if (!mediaElementUsesSource(menuVideo, pair.video)) {
        menuVideo.src = pair.video;
        menuVideo.load?.();
      }
    } catch (_) {}

    let audio = state.intro.musicAudio;
    if (!audio || !mediaElementUsesSource(audio, pair.audio)) {
      try { audio?.pause?.(); } catch (_) {}
      audio = new Audio(pair.audio);
      audio.preload = "auto";
      audio.loop = true;
      audio.volume = 0;
    ...
    const [videoStatus, audioStatus] = await Promise.all([
      inspectMediaSource(menuVideo),
      inspectMediaSource(audio),
    ]);

    lastResult = {
      valid: videoStatus.valid !== false && audioStatus.valid !== false,
      video: videoStatus,
      audio: audioStatus,
      pair,
    };
    ...
    if (lastResult.valid) {
      rememberResolvedIntroPair(set, pair);
      state.intro.menuVideoReady = Boolean(videoStatus.ready);
      state.intro.menuAudioReady = Boolean(audioStatus.ready);
      state.intro.selectedPairStatus = { video: videoStatus, audio: audioStatus, source: pair.source };
      return lastResult;
    }
  }

  state.intro.menuVideoReady = false;
  state.intro.menuAudioReady = false;
  state.intro.selectedPairStatus = {
    video: lastResult.video,
    audio: lastResult.audio,
    source: lastResult.pair?.source || "unavailable",
  };
  return lastResult;
}

async function beginSilentIntroPrecache() {
  if (state.intro.silentPrecachePromise) return state.intro.silentPrecachePromise;
  state.intro.silentPrecacheStarted = true;
  const selected = chooseIntroMenuSet();

  state.intro.silentPrecachePromise = Promise.all([
    loadSelectedIntroPair(selected),
    preloadFirstAvailable({ sources: INTRO_ASSETS.logoVideo, type: "video" }),
    preloadFirstAvailable({ sources: INTRO_ASSETS.logoAudio, type: "audio" }),
    preloadFirstAvailable({ sources: [INTRO_ASSETS.overlayLogo], type: "video" }),
    preloadFirstAvailable({ sources: [INTRO_ASSETS.loadingVideo], type: "video" }),
  ]).then(async results => {
    const selectedPair = results[0];
    if (selectedPair?.valid === false && selected.id !== INTRO_MENU_SETS[0].id) {
      state.intro.selectedSet = INTRO_MENU_SETS[0];
      state.intro.selectedSetIndex = 0;
      await loadSelectedIntroPair(INTRO_MENU_SETS[0]);
    }
    void scheduleGeneralSilentPrecache();
    scheduleAlternativeIntroPrecache();
    return true;
  }).catch(() => {
    void scheduleGeneralSilentPrecache();
    scheduleAlternativeIntroPrecache();
  ...
  return state.intro.silentPrecachePromise;
}

function scheduleAlternativeIntroPrecache() {
  if (state.intro.alternativesPrecacheStarted) return state.intro.alternativesPrecachePromise;
  state.intro.alternativesPrecacheStarted = true;
  const selected = currentIntroMenuSet();
  const alternatives = INTRO_MENU_SETS.filter(item => item.id !== selected.id);
  const task = async () => {
  ...
  state.intro.alternativesPrecachePromise = new Promise(resolve => {
    ...
    window.setTimeout(queueWhenIdle, 9000);
  });
  return state.intro.alternativesPrecachePromise;
}

async function activateDefaultIntroPairAfterError() {
  const current = currentIntroMenuSet();
  const currentCandidates = introSetCandidatePairs(current);
  const active = resolvedIntroPair(current);
  const activeIndex = currentCandidates.findIndex(pair =>
    absoluteMediaUrl(pair.video) === absoluteMediaUrl(active.video) &&
    absoluteMediaUrl(pair.audio) === absoluteMediaUrl(active.audio)
  );

  for (const pair of currentCandidates.slice(Math.max(0, activeIndex + 1))) {
    rememberResolvedIntroPair(current, pair);
    const result = await loadSelectedIntroPair(current, pair);
    if (result?.valid) return current;
  }

  const fallback = INTRO_MENU_SETS[0];
  state.intro.selectedSet = fallback;
  state.intro.selectedSetIndex = 0;
  state.intro.resolvedSetId = null;
  await loadSelectedIntroPair(fallback);
  return fallback;
}

async function playSelectedIntroMenuVideo() {
  const video = introMenuVideoElement();
  if (!video) return "missing";
  let set = currentIntroMenuSet();
  let pair = resolvedIntroPair(set);
  if (!mediaElementUsesSource(video, pair.video)) {
  ...
  try {
    ...
    if (video.error) {
      set = await activateDefaultIntroPairAfterError();
      pair = resolvedIntroPair(set);
      playIntroMusic();
      if (!mediaElementUsesSource(video, pair.video)) {
        video.src = pair.video;
        video.load?.();
...
function stopSelectedIntroMenuVideo(clearSource = false) {
  const video = introMenuVideoElement();
  if (!video) return;
  try {
    video.pause();
...
function playIntroMusic() {
  const set = currentIntroMenuSet();
  const pair = resolvedIntroPair(set);
  let audio = state.intro.musicAudio;
  if (!audio || !mediaElementUsesSource(audio, pair.audio)) {
    try { audio?.pause?.(); } catch (_) {}
    audio = new Audio(pair.audio);
...
async function runLoadingIntroVideo() {
  const overlay = createIntroOverlay();
  const video = introVideoElement();
  stopSelectedIntroMenuVideo(false);
...
function finishIntroSequence() {
  ...
  stopSelectedIntroMenuVideo(true);
...
async function startIntroSequence() {
  ...
  await beginSilentIntroPrecache();
  ...
  void playSelectedIntroMenuVideo();
...
function createPlayerSlotElement(team, index) {
  const slot = document.createElement("article");
  slot.className = `player-slot team-${team.toLowerCase()} slot-empty`;
...
function ensurePlayerSlotElements(team, container, count) {
  if (!container) return [];
  while (container.children.length > count) container.lastElementChild?.remove();
  while (container.children.length < count) {
    container.appendChild(createPlayerSlotElement(team, container.children.length));
  }
  return Array.from(container.children);
}

function updatePlayerSlotPortrait(portrait, character, previewCharacter) {
  const visualCharacter = character || previewCharacter || null;
  const visualKey = visualCharacter ? `${character ? "pick" : "preview"}:${visualCharacter.name}` : "empty";
  if (portrait.dataset.visualKey === visualKey) return;

  portrait.dataset.visualKey = visualKey;
  portrait.replaceChildren();

  if (visualCharacter) {
    portrait.appendChild(makeImage([thumbPath(visualCharacter.name), legacyPath(visualCharacter.name)], "", visualCharacter.name));
    return;
  }
...
function updatePlayerSlotElement(slot, team, index, turn) {
  const pick = state.picks[team][index] || null;
  const isActivePickSlot = turn?.type === "pick" && turn.team === team && index === state.picks[team].length;
  const previewPick = isActivePickSlot ? currentRoulettePreviewCharacter(turn) : null;
  const hasPreview = Boolean(!pick && previewPick);

  slot.dataset.slotIndex = String(index);
  slot.dataset.state = pick ? "filled" : hasPreview ? "preview" : isActivePickSlot ? "active" : "empty";
  slot.classList.toggle("slot-filled", Boolean(pick));
  slot.classList.toggle("slot-empty", !pick && !hasPreview);
  slot.classList.toggle("slot-preview", hasPreview);
  slot.classList.toggle("active-turn", isActivePickSlot);
  slot.classList.toggle("roulette-preview-slot", hasPreview);

  const flashKey = pick && state.flashPick === pick.name ? `${state.draftSessionId}:${state.turnIndex}:${pick.name}` : "";
  if (flashKey && slot.dataset.flashKey !== flashKey) {
    slot.classList.remove("pick-flash");
    void slot.offsetWidth;
    slot.classList.add("pick-flash");
    slot.dataset.flashKey = flashKey;
  } else if (!flashKey) {
    slot.classList.remove("pick-flash");
    delete slot.dataset.flashKey;
  }

  const portrait = slot.querySelector(".slot-portrait");
  const playerName = slot.querySelector(".slot-name");
  const characterName = slot.querySelector(".slot-character");

  updatePlayerSlotPortrait(portrait, pick, previewPick);
  playerName.textContent = state.players[team][index] || `PLAYER ${index + 1}`;
  ...
  slot.setAttribute("aria-label", `${playerName.textContent}: ${characterName.textContent}`);
}

function renderTeamSlots(team, container) {
  const turn = currentTurn();
  const slots = ensurePlayerSlotElements(team, container, activeTeamSize());
  slots.forEach((slot, index) => updatePlayerSlotElement(slot, team, index, turn));
}
  ...
  card.addEventListener("click", () => {
    ...
    if (currentRoomCode) {
      pushOnlineDraftPatch({
        ...
        preselectLocked: true,
        actionEvent: createOnlineActionEvent("preselect", currentTurn()?.team || null, state.selected, { source: "locked-click" }),
      });
    }
...
function clearPreselection() {
  ...
  if (currentRoomCode) {
    pushOnlineDraftPatch({
      selected: serializeCharacterForOnline(null),
      preselectLocked: false,
    });
  }
  renderCharacterSelectionLight();
...
function updateCharacterCardStates() {
  const turn = currentTurn();
  const selectedName = state.selected?.name || null;
  const bannedSet = new Set(bannedNames());
  const pickedTeams = new Map();
  state.picks.A.forEach(character => pickedTeams.set(character.name, "A"));
  state.picks.B.forEach(character => pickedTeams.set(character.name, "B"));

  document.querySelectorAll(".character-card").forEach(card => {
    const character = characters.find(item => item.name === card.dataset.name);
    if (!character) return;
    const pickedTeam = pickedTeams.get(character.name) || null;
    const isSelected = selectedName === character.name;

    card.classList.toggle("disabled", !isCharacterAvailable(character, turn));
    card.classList.toggle("banned", bannedSet.has(character.name));
    card.classList.toggle("picked", Boolean(pickedTeam));
    card.classList.toggle("pick-flash", state.flashPick === character.name);
    card.classList.toggle("ban-flash", state.flashBan === character.name);
    card.classList.toggle("roulette-highlight", state.roulette.active && state.roulette.highlightedName === character.name);
    card.classList.toggle("roulette-winner", Boolean(state.roulette.finalName) && state.roulette.finalName === character.name);
    card.classList.toggle("selected", isSelected);
    card.classList.remove("selected-team-a", "selected-team-b", "picked-team-a", "picked-team-b", "preselect-locked");

    if (pickedTeam) card.classList.add(`picked-team-${pickedTeam.toLowerCase()}`);
    if (isSelected) {
      card.classList.add(`selected-${currentTeamClass(turn)}`);
      if (state.preselectLocked) card.classList.add("preselect-locked");
    }
  });
}

function renderDraftStateLight(options = {}) {
  renderTurnInfo();
  renderCaptainTurnBanner();
  renderStageCharacters();
  updateCharacterCardStates();
  if (options.refreshPreviewPanels) {
    renderSlots();
    renderBans();
  }
  renderSelected();
...
function renderCharacterSelectionLight() {
  updateCharacterCardStates();
  renderStageCharacters();
  renderSelected();
}

function renderConfirmedActionState() {
  renderTurnInfo();
  renderSlots();
  renderBans();
  renderCaptainTurnBanner();
...
function updateCharacterRoulettePreview(character) {
  ...
  updateCharacterRouletteClasses();
  renderStageCharacters();
  if (currentTurn()?.type === "pick") renderSlots();
  else renderBans();
  renderSelected();
}
...
function onlineUnavailableMessage() {
  return t("service_unavailable_body");
}

let appNoticeSequence = 0;
const appNoticeTimers = new WeakMap();

function noticeTitleForType(type = "info") {
  const key = {
    error: "notice_error_title",
    warning: "notice_attention_title",
    success: "notice_success_title",
    info: "notice_info_title",
  }[type] || "notice_info_title";
  return t(key);
}

function removeAppNotice(notice, immediate = false) {
  if (!notice) return;
  const timer = appNoticeTimers.get(notice);
  if (timer) window.clearTimeout(timer);
  appNoticeTimers.delete(notice);
  if (immediate) {
    notice.remove();
    return;
  }
...
function showAppNotice(message, options = {}) {
  ...
  const type = ["error", "warning", "success", "info"].includes(options.type) ? options.type : "info";
  const notice = document.createElement("article");
  notice.className = `app-notification is-${type}`;
  notice.dataset.noticeId = String(++appNoticeSequence);
  ...
  title.textContent = String(options.title || noticeTitleForType(type));
  ...
  close.type = "button";
  close.setAttribute("aria-label", t("notice_close"));
  close.textContent = "×";
  close.addEventListener("click", () => removeAppNotice(notice));

  notice.append(icon, copy, close);
  stack.prepend(notice);

  while (stack.children.length > 4) removeAppNotice(stack.lastElementChild, true);

  const duration = Number(options.duration);
  const timeout = Number.isFinite(duration) ? duration : type === "error" ? 6200 : 4800;
  if (timeout > 0) {
    const timer = window.setTimeout(() => removeAppNotice(notice), timeout);
    appNoticeTimers.set(notice, timer);
  }

  return notice;
}

let pendingOnlineServiceAction = null;
let onlineServiceConnectionCheck = null;
let onlineServiceGateBusy = false;
let onlineServiceNoticeHideTimer = null;

function setOnlineActionButtonsBusy(busy) {
  ["create-room", "join-room", "join-name-confirm"].forEach(id => {
...
function updateOnlineServiceNoticeLanguage() {
  const notice = document.getElementById("online-service-notice");
  if (!notice) return;
  const stateName = notice.dataset.state || "unavailable";
  const title = document.getElementById("online-service-title");
  const message = document.getElementById("online-service-message");
  const kicker = document.getElementById("online-service-kicker");
  const retry = document.getElementById("online-service-retry");
  const dismiss = document.getElementById("online-service-dismiss");

  if (kicker) kicker.textContent = t("service_status_kicker");
  if (dismiss) {
    dismiss.textContent = t("service_dismiss");
    dismiss.disabled = stateName === "checking";
  }

  if (stateName === "connected") {
    if (title) title.textContent = t("service_restored_title");
    if (message) message.textContent = t("service_restored_body");
    if (retry) {
      retry.textContent = t("service_connected_action");
      retry.disabled = true;
    }
  } else if (stateName === "checking") {
    if (title) title.textContent = t("service_retrying_title");
    if (message) message.textContent = t("service_retrying_body");
    if (retry) {
      retry.textContent = t("service_retrying");
      retry.disabled = true;
    }
  } else {
    if (title) title.textContent = t("service_unavailable_title");
    if (message) message.textContent = onlineUnavailableMessage();
    if (retry) {
      retry.textContent = t("service_reconnect");
      retry.disabled = false;
    }
  }
}

function setOnlineServiceNoticeState(stateName, options = {}) {
  const notice = document.getElementById("online-service-notice");
  if (!notice) {
    if (stateName === "unavailable") showAppNotice(onlineUnavailableMessage(), { type: "error", duration: 0 });
    return;
  }

  if (onlineServiceNoticeHideTimer) {
    window.clearTimeout(onlineServiceNoticeHideTimer);
    onlineServiceNoticeHideTimer = null;
  }

  notice.dataset.state = stateName;
  notice.classList.remove("hidden", "is-hiding", "is-connected", "is-checking", "is-retrying", "notice-replay");
  notice.classList.toggle("is-connected", stateName === "connected");
  notice.classList.toggle("is-checking", stateName === "checking");
  if (options.replay !== false) {
    void notice.offsetWidth;
    notice.classList.add("notice-replay");
    window.setTimeout(() => notice.classList.remove("notice-replay"), 560);
  }
  updateOnlineServiceNoticeLanguage();
}

function showOnlineServiceNotice() {
  setOnlineServiceNoticeState("unavailable");
}

function hideOnlineServiceNotice(options = {}) {
  const notice = document.getElementById("online-service-notice");
  if (options.clearPending !== false) pendingOnlineServiceAction = null;
  if (!notice || notice.classList.contains("hidden")) return;
  notice.classList.add("is-hiding");
  onlineServiceNoticeHideTimer = window.setTimeout(() => {
    notice.classList.add("hidden");
    notice.classList.remove("is-hiding", "is-connected", "is-checking", "is-retrying", "notice-replay");
    notice.dataset.state = "unavailable";
    onlineServiceNoticeHideTimer = null;
  }, 280);
}

function waitForOnlineServiceConnection(database, timeoutMs = 5200) {
  return new Promise(resolve => {
    let finished = false;
    ...
    const finish = value => {
      ...
      try { connectedRef.off("value", handleValue); } catch (_) {}
      resolve(Boolean(value));
    };
    const handleValue = snapshot => {
      if (snapshot?.val?.() === true) finish(true);
    };
    const timeoutId = window.setTimeout(() => finish(false), Math.max(1200, Number(timeoutMs) || 5200));

    try {
      connectedRef.on("value", handleValue, () => finish(false));
    } catch (_) {
      finish(false);
    }
  });
...
async function verifyOnlineServiceConnection(options = {}) {
  if (onlineServiceConnectionCheck) return onlineServiceConnectionCheck;

  onlineServiceConnectionCheck = (async () => {
    if (typeof navigator !== "undefined" && navigator.onLine === false) return false;

    try {
      if (!getRealtimeDatabase() && typeof window.initRpmodsServices === "function") {
        window.initRpmodsServices();
      }
    } catch (error) {
      console.warn("RPmods Services no pudo inicializarse durante la comprobación.", error);
    }
    ...
    return waitForOnlineServiceConnection(database, options.timeoutMs);
  })();

  try {
    return await onlineServiceConnectionCheck;
  } finally {
    onlineServiceConnectionCheck = null;
  }
}

async function checkOnlineServiceAndContinue(action = null, options = {}) {
  if (typeof action === "function") {
    pendingOnlineServiceAction = {
      action,
      source: String(options.source || "online-action"),
    };
  }

  if (onlineServiceGateBusy) return false;
  onlineServiceGateBusy = true;
  setOnlineActionButtonsBusy(true);
  setOnlineServiceNoticeState("checking");

  const startedAt = performance.now();
  const connected = await verifyOnlineServiceConnection({ timeoutMs: options.timeoutMs || 5200 });
  const elapsed = performance.now() - startedAt;
  if (elapsed < 520) await new Promise(resolve => window.setTimeout(resolve, 520 - elapsed));

  if (!connected) {
    setOnlineServiceNoticeState("unavailable", { replay: false });
    onlineServiceGateBusy = false;
    setOnlineActionButtonsBusy(false);
    return false;
  }

  setOnlineServiceNoticeState("connected", { replay: false });
  const pending = pendingOnlineServiceAction;
  pendingOnlineServiceAction = null;
  onlineServiceGateBusy = false;
  setOnlineActionButtonsBusy(false);
  ...
  try {
    ...
    console.error("La acción online no pudo completarse después de verificar RPmods Services.", error);
    const stillConnected = await verifyOnlineServiceConnection({ timeoutMs: 1800 });
    if (!stillConnected) {
      pendingOnlineServiceAction = pending;
      setOnlineServiceNoticeState("unavailable");
      return false;
    }
    showAppNotice(t("service_action_failed"), { type: "error" });
    return false;
  }

  onlineServiceNoticeHideTimer = window.setTimeout(() => hideOnlineServiceNotice({ clearPending: false }), 1050);
  return true;
}

async function retryOnlineServiceConnection() {
  const retryButton = document.getElementById("online-service-retry");
  if (retryButton?.disabled || onlineServiceGateBusy) return;
  const connected = await checkOnlineServiceAndContinue(null, { source: "reconnect" });
  if (!connected) {
    const notice = document.getElementById("online-service-notice");
    if (notice) {
      notice.classList.remove("retry-failed");
      void notice.offsetWidth;
      notice.classList.add("retry-failed");
      window.setTimeout(() => notice.classList.remove("retry-failed"), 520);
    }
  }
}

function setupAppNotificationUI() {
  document.getElementById("online-service-retry")?.addEventListener("click", retryOnlineServiceConnection);
  document.getElementById("online-service-dismiss")?.addEventListener("click", () => hideOnlineServiceNotice({ clearPending: true }));
}
...
async function playOnlineCharacterRouletteSequence(event) {
  const sequence = Array.isArray(event?.sequence) ? event.sequence.map(characterFromOnlineValue).filter(Boolean) : [];
  if (!sequence.length) return;
  const delays = Array.isArray(event.delays) ? event.delays.map(value => Math.max(40, Number(value) || 0)) : [];
  const playbackId = event.id;
  onlineRemoteRoulettePlaybackId = playbackId;
  state.roulette.active = true;
  state.locked = true;

  let elapsed = 0;
  for (let index = 0; index < sequence.length; index += 1) {
    const stepDelay = delays[index] || 90;
    const targetAt = Number(event.startedAt || onlineNow()) + elapsed;
    elapsed += stepDelay;
    if (onlineNow() >= targetAt + stepDelay && index < sequence.length - 1) continue;
    const waitMs = Math.max(0, targetAt - onlineNow());
    if (waitMs) await delay(waitMs);
    if (onlineRemoteRoulettePlaybackId !== playbackId) return;
    const character = sequence[index];
    state.roulette.highlightedName = character.name;
    updateCharacterRoulettePreview(character);
    audioPlay(sounds.roulette, 0.82, "sfx");
  }
}

async function playOnlineMapRouletteSequence(event) {
  const sequence = Array.isArray(event?.sequence) ? event.sequence.map(mapFromOnlineValue).filter(Boolean) : [];
  if (!sequence.length) return;
  const delays = Array.isArray(event.delays) ? event.delays.map(value => Math.max(40, Number(value) || 0)) : [];
  const playbackId = event.id;
  onlineRemoteMapPlaybackId = playbackId;
  ...
  for (let index = 0; index < sequence.length; index += 1) {
    ...
    const waitMs = Math.max(0, targetAt - onlineNow());
    if (waitMs) await delay(waitMs);
    if (onlineRemoteMapPlaybackId !== playbackId) return;
    state.mapRoulette.highlightedId = sequence[index].id;
...
function handleOnlineRouletteEvent(event) {
  ...
  if (event.type === "rouletteSequence") {
    void playOnlineCharacterRouletteSequence(event);
  } else if (event.type === "rouletteSelected") {
    onlineRemoteRoulettePlaybackId = null;
...
function handleOnlineMapEvent(event) {
  ...
  if (event.byClientId === onlineClientId()) return;
  if (event.type === "mapSequence") {
    void playOnlineMapRouletteSequence(event);
  } else if (event.type === "mapSelected") {
    onlineRemoteMapPlaybackId = null;
...
function pushOnlineDraftState(extra = {}) {
  ...
  roomRef.child("draftState").update(currentOnlineDraftPayload(extra)).catch(error => {
    console.warn("RPmods Services no pudo sincronizar el estado online.", error);
  });
}

function pushOnlineDraftPatch(patch = {}) {
  if (suppressOnlinePush || !currentRoomCode) return;
  if (!state.draftActive && !patch.phase && !patch.force) return;
  const roomRef = roomRefFor(currentRoomCode);
  if (!roomRef) return;

  const payload = Object.fromEntries(
    Object.entries(patch).filter(([key, value]) => key !== "force" && value !== undefined),
  );
  payload.updatedAt = onlineNow();

  roomRef.child("draftState").update(payload).catch(error => {
    console.warn("RPmods Services no pudo sincronizar la actualización online.", error);
  });
}
...
async function assignAdvancedSlot(team, slotKey, clientId) {
  ...
  try {
    ...
    showAppNotice("No se pudo asignar el jugador. Intenta de nuevo.", { type: "error" });
  }
}
...
async function updateHostOnlineName(name) {
  ...
  try {
    ...
    showAppNotice(t("host_name_error"), { type: "error" });
  }
}
...
async function fillClassicTestingBots() {
  ...
  try {
    ...
    showAppNotice(t("testing_bots_error"), { type: "error" });
  }
}
...
function syncDraftStateFromRoom(data = {}) {
  ...
  if (draftScreenActive) {
    if (!characterGrid?.children?.length) renderAll();
    else renderDraftStateLight({ refreshPreviewPanels: remoteTurnChanged || remoteSessionChanged || picksBansChanged });
  }
...
async function runMapRoulette(options = {}) {
  ...
  const sequence = Array.from({ length: totalSteps }, () => randomFrom(maps));
  const delays = sequence.map((_, step) => weightedRandomDelay(step, totalSteps));
  const startedAt = onlineNow();
  let selected = sequence[sequence.length - 1];
  ...
  if (currentRoomCode) {
    pushOnlineDraftPatch({
      ...
      mapRoulette: serializeMapRouletteForOnline(state.mapRoulette),
      mapEvent: {
        id: `mapSequence_${startedAt}_${Math.random().toString(36).slice(2, 7)}`,
        type: "mapSequence",
        sequence: sequence.map(map => map.id),
        delays,
        startedAt,
        byClientId: onlineClientId(),
      },
...
async function assignOnlineCaptain(team, clientId) {
  ...
  try {
    ...
    showAppNotice(t("captain_assign_error"), { type: "error" });
  }
}
...
async function markCurrentPlayerReady() {
  ...
  try {
    ...
    showAppNotice(t("ready_check_error"), { type: "error" });
  }
}
  ...
  if (readyCheck.status === "loading") {
    ...
    setTimeout(async () => {
      try {
        ...
        console.error(error);
        onlineReadyStartKey = null;
        showAppNotice(t("online_start_error"), { type: "error" });
      }
    }, delayMs);
  }
}
...
function setupOnlineControls() {
  ...
  if (copy) copy.addEventListener("click", async () => {
    ...
    try {
      ...
      showAppNotice(`${t("online_room_code")}: ${currentRoomCode}`, { type: "info", duration: 9000 });
    }
  });
  ...
  if (onlineStartBtn) {
    onlineStartBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        console.error(error);
        if (error?.message === "advanced_slots_missing") showAppNotice(t("ready_check_missing_players"), { type: "warning" });
        else if (error?.message === "captains_missing") showAppNotice(t("lobby_alert_assign_captains"), { type: "warning" });
        else if (error?.message === "captains_unavailable") showAppNotice(t("lobby_alert_missing_captains"), { type: "warning" });
        else showAppNotice(t("online_start_error"), { type: "error" });
      }
    });
  }
}
...
function handleRoomClosed(roomCode, options = {}) {
  ...
  switchScreen(setupScreen);
  startMusic("menu");
  if (!options.silent) showAppNotice(t("room_closed_by_host"), { type: "warning", duration: 7000 });
}

function watchRoomDeletion(roomCode) {
  const roomRef = roomRefFor(roomCode);
  if (!roomRef || onlineRoomDeletionListenerCode === roomCode) return;
  try { onlineRoomDeletionRef?.off("value"); } catch (_) {}
  onlineRoomDeletionListenerCode = roomCode;
  const closedRef = roomRef.child("closed");
  onlineRoomDeletionRef = closedRef;
  closedRef.on("value", (snapshot) => {
    if (currentRoomCode === roomCode && snapshot.val() === true) handleRoomClosed(roomCode);
  });
}
...
/* Compatibility guard: v3.4.4 referenced a helper that is not present in the stable base. */
if (typeof applyOnlineDraftState === "undefined") {
  var applyOnlineDraftState = function rpmodsApplyOnlineDraftStateCompatibility(data = {}) {
    return syncDraftStateFromRoom(data);
  };
}

/* RPmods v3.4.4 — Map-first Stable Flow + Assist Request Fix */
(function() {
  const RP344_VERSION = "3.4.4";
  const CHIBI_SIZE_STORAGE_KEY = "rpmods_map_chibi_size_v1";

  try { if (factions?.urbino) factions.urbino.label = "Urbino"; } catch (_) {}

  function rp344CurrentChibiSize() {
    try {
      const raw = Number(localStorage.getItem(CHIBI_SIZE_STORAGE_KEY));
      return Math.max(90, Math.min(220, Number.isFinite(raw) ? raw : 150));
    } catch (_) { return 150; }
  }

  function rp344ApplyChibiSize(size = rp344CurrentChibiSize()) {
    const safe = Math.max(90, Math.min(220, Number(size) || 150));
    document.documentElement.style.setProperty("--rpmods-map-chibi-size", `${safe}px`);
    const value = document.getElementById("map-chibi-size-value");
    if (value) value.textContent = `${safe}px`;
    const range = document.getElementById("map-chibi-size-range");
    if (range && String(range.value) !== String(safe)) range.value = String(safe);
  }

  function rp344EnsureConfigPanel() {
    const introPanel = document.getElementById("intro-settings-panel");
    if (!introPanel) return;
    introPanel.classList.remove("hidden");
    if (!document.getElementById("map-chibi-settings-panel")) {
      const panel = document.createElement("section");
      panel.id = "map-chibi-settings-panel";
      panel.className = "intro-settings-panel map-chibi-settings-panel";
      panel.innerHTML = `
        <div class="subconfig-heading intro-settings-heading">
          <span>Selección de mapa</span>
          <strong>Chibi de eliminación</strong>
        </div>
        <label class="subconfig-row map-chibi-size-row" for="map-chibi-size-range">
          <div class="subconfig-copy">
            <span>Tamaño del chibi</span>
            <small>Ajusta el PNG de caída y el PNG de impacto. Recomendado: 150px para archivos 300x300.</small>
          </div>
          <div class="subconfig-control">
            <input id="map-chibi-size-range" class="turn-time-range" type="range" min="90" max="220" step="5" value="150" />
            <div class="turn-time-meta"><span id="map-chibi-size-value" class="volume-value">150px</span></div>
          </div>
        </label>
        <p class="intro-selection-status">Rutas usadas: <code>img/ui/map_chibi_fall.png</code>, <code>img/ui/map_chibi_land.png</code> y <code>audio/map_vote_chibi_1.ogg</code> hasta <code>audio/map_vote_chibi_8.ogg</code>.</p>
      `;
      introPanel.insertAdjacentElement("afterend", panel);
      panel.querySelector("#map-chibi-size-range")?.addEventListener("input", event => {
        const size = Math.max(90, Math.min(220, Number(event.currentTarget.value) || 150));
        try { localStorage.setItem(CHIBI_SIZE_STORAGE_KEY, String(size)); } catch (_) {}
        rp344ApplyChibiSize(size);
      });
    }
    rp344ApplyChibiSize();
  }

  function rp344TeamOfClient() {
    return currentOnlineTeamLetter?.() || null;
  }

  function rp344OwnAssignment() {
    try { return advancedAssignmentForClient(onlineClientId(), state.onlineSlots); } catch (_) { return null; }
  }

  function rp344CanRequestDuringTurn() {
    const turn = currentTurn();
    if (!currentRoomCode || !isAdvancedDraftConfig() || !turn || turn.type !== "pick") return false;
    const own = rp344OwnAssignment();
    if (!own || own.team !== turn.team) return false;
    if (own.slotKey === turn.slotKey) return false;
    return /^player[3-5]$/i.test(String(own.slotKey || ""));
  }

  const previousPreselectCharacter344 = preselectCharacter;
  preselectCharacter = function rpmodsPreselectCharacterV344(character, options = {}) {
    if (rp344CanRequestDuringTurn() && !canControlCurrentTurn()) {
      if (!character || !isCharacterAvailable(character, currentTurn())) return;
      const previousName = state.selected?.name || null;
      if (previousName === character.name) return;
      state.selected = character;
      state.preselectLocked = true;
      audioPlay(sounds.select, options.source === "hover" ? 0.45 : 0.7, "sfx");
      renderCharacterSelectionLight();
      return;
    }
    return previousPreselectCharacter344(character, options);
  };

  function rp344EnsureRequestActionButton() {
    const confirmButton = document.getElementById("confirm-action");
    if (!confirmButton) return null;
    let button = document.getElementById("rpmods-request-pick-action");
    if (!button) {
      button = document.createElement("button");
      button.id = "rpmods-request-pick-action";
      button.type = "button";
      button.className = "random-selection-button rpmods-request-pick-action hidden";
      button.textContent = "PEDIR";
      button.title = "Pedir este laminante al capitán o subcapitán";
      button.setAttribute("aria-label", "Pedir este laminante al capitán o subcapitán");
      confirmButton.insertAdjacentElement("afterend", button);
      button.addEventListener("click", event => {
        event.preventDefault();
        event.stopPropagation();
        rp344SubmitPickRequest();
      });
    }
    return button;
  }

  function rp344RequestKey(team, index) { return `${team}:${index}`; }

  function rp344NormalizeRequests(value = {}) {
    if (!value || typeof value !== "object") return {};
    return Object.fromEntries(Object.entries(value).filter(([, req]) => req && typeof req === "object" && req.characterName));
  }

  function rp344SyncRequestsOnline() {
    if (!currentRoomCode) return;
    pushOnlineDraftPatch({ rpmodsRequests: state.rpmodsRequests || {} });
  }

  function rp344SubmitPickRequest() {
    const turn = currentTurn();
    if (!rp344CanRequestDuringTurn()) {
      showAppNotice("Solo puedes pedir laminante cuando tu equipo está seleccionando.", { type: "warning" });
      return;
    }
    if (!state.selected || !isCharacterAvailable(state.selected, turn)) {
      showAppNotice("Primero marca el laminante que quieres pedir.", { type: "warning" });
      return;
    }
    const own = rp344OwnAssignment();
    const slotIndex = advancedSlotIndex(own.slotKey);
    state.rpmodsRequests = state.rpmodsRequests || {};
    state.rpmodsRequests[rp344RequestKey(own.team, slotIndex)] = {
      characterName: state.selected.name,
      requestedBy: own.slot?.name || state.players?.[own.team]?.[slotIndex] || "Compañero",
      team: own.team,
      slotIndex,
      slotKey: own.slotKey,
      at: onlineNow ? onlineNow() : Date.now(),
    };
    rp344SyncRequestsOnline();
    showAppNotice(`Pedido enviado: ${state.selected.name}.`, { type: "success" });
    renderAll();
  }

  const previousRenderSelected344 = renderSelected;
  renderSelected = function rpmodsRenderSelectedV344() {
    previousRenderSelected344();
    const button = rp344EnsureRequestActionButton();
    if (!button) return;
    const turn = currentTurn();
    const canRequest = rp344CanRequestDuringTurn();
    const selectedOk = Boolean(state.selected && turn && isCharacterAvailable(state.selected, turn));
    button.classList.toggle("hidden", !(canRequest && selectedOk && !state.locked && !state.roulette.active));
    const confirm = document.getElementById("confirm-action");
    if (confirm && canRequest) confirm.classList.add("hidden");
  };

  const previousCurrentOnlineDraftPayload344 = currentOnlineDraftPayload;
  currentOnlineDraftPayload = function rpmodsCurrentOnlineDraftPayloadV344(extra = {}) {
    return previousCurrentOnlineDraftPayload344({ rpmodsRequests: state.rpmodsRequests || {}, ...extra });
  };

  const previousBlankOnlineDraftState344 = blankOnlineDraftState;
  blankOnlineDraftState = function rpmodsBlankOnlineDraftStateV344(startPayload = {}) {
    return previousBlankOnlineDraftState344({ rpmodsRequests: {}, ...startPayload });
  };

  const previousApplyOnlineDraftState344 = applyOnlineDraftState;
  applyOnlineDraftState = function rpmodsApplyOnlineDraftStateV344(data = {}) {
    const draftState = data?.draftState || {};
    if (Object.prototype.hasOwnProperty.call(draftState, "rpmodsRequests")) {
      state.rpmodsRequests = rp344NormalizeRequests(draftState.rpmodsRequests);
    }
    return previousApplyOnlineDraftState344(data);
  };

  function rp344BeginDraftAfterPredraftMap() {
    if (!isDraftSessionActive()) return;
    state.mapSelectionContext = "predraft-done";
    state.locked = false;
    state.selected = null;
  ...
  function rp344FinishMapSelection() {
    updateMapRouletteClasses();
    updateSelectedMapCopy();
    if (state.mapSelectionContext === "predraft") {
      rp344BeginDraftAfterPredraftMap();
    } else {
      showSummaryIntro();
    }
  }
  window.rpFinishMapSelectionContext = rp344FinishMapSelection;

  function rp344AudioCandidates(base) {
    if (!base) return [];
    if (/\.(ogg|mp3|mp4)$/i.test(base)) return [base];
    return [`${base}.ogg`, `${base}.mp3`, `${base}.mp4`];
  }
  function rp344PlayChibiVoice() {
    unlockMediaPlayback(true);
    const first = "audio/map_vote_chibi_1.ogg";
    const pool = [first, ...Array.from({ length: 7 }, (_, idx) => `audio/map_vote_chibi_${idx + 2}`)];
    const src = pool[Math.floor(Math.random() * pool.length)] || first;
    audioPlayFromSourceList(rp344AudioCandidates(src), 1, "sfx");
  }

  function rp344EnsureChibiOverlay() {
    let overlay = document.getElementById("map-chibi-overlay-v344");
    if (overlay) return overlay;
    overlay = document.createElement("div");
    overlay.id = "map-chibi-overlay-v344";
    overlay.className = "map-chibi-overlay-v344 hidden";
    overlay.innerHTML = `<img class="map-chibi-fall-v344" alt="chibi" src="img/ui/map_chibi_fall.png"><img class="map-chibi-land-v344" alt="chibi" src="img/ui/map_chibi_land.png">`;
    document.body.appendChild(overlay);
    return overlay;
  }

  async function rp344DropChibi(mapId, isFinalPause = false) {
    const card = mapGrid?.querySelector(`.map-card[data-map-id="${mapId}"]`);
    if (!card) return;
    if (isFinalPause) await delay(2000);
    const overlay = rp344EnsureChibiOverlay();
    const rect = card.getBoundingClientRect();
    overlay.style.left = `${Math.round(rect.left + rect.width / 2)}px`;
    overlay.style.top = `${Math.round(rect.top + rect.height / 2 - 4)}px`;
    overlay.classList.remove("hidden", "is-landing", "is-dropping");
    void overlay.offsetWidth;
    overlay.classList.add("is-dropping");
    await delay(540);
  ...
  function rp344EliminationPlan(total) {
    const remove = Math.max(0, total - 1);
    if (total === 9) return [3, 2, 2, 1];
    const plan = [];
    let left = remove;
  ...
  runMapRoulette = async function rpmodsRunMapRouletteV344(options = {}) {
    const sessionId = state.draftSessionId;
    if (!isDraftSessionActive(sessionId) || state.mapRoulette.active || !maps.length) return;
    const config = currentDraftConfig();
    state.mapRoulette.active = true;
    ...
    if (config.mapRandomMode === "classic-random") {
      ...
      for (let i = 0; i < 18; i += 1) {
        ...
        state.mapRoulette.highlightedId = selected.id;
        updateMapRouletteClasses();
        audioPlay(sounds.mapRoulette || sounds.roulette, 0.7, "sfx");
        await delay(65 + i * 9);
        if (!isDraftSessionActive(sessionId)) return;
      }
      state.selectedMap = selected;
      ...
      state.mapRoulette.active = false;
      pushOnlineDraftState({ selectedMap: { id: selected.id, name: selected.name }, mapRoulette: serializeMapRouletteForOnline(state.mapRoulette) });
      rp344FinishMapSelection();
      return;
    }

    const pool = [...maps];
    const plan = rp344EliminationPlan(pool.length);
    for (let groupIndex = 0; groupIndex < plan.length; groupIndex += 1) {
      const count = plan[groupIndex];
      for (let i = 0; i < count; i += 1) {
        if (pool.length <= 1) break;
        ...
        state.mapRoulette.highlightedId = target.id;
        updateMapRouletteClasses();
        await rp344DropChibi(target.id, groupIndex === plan.length - 1 && i === count - 1);
        if (!isDraftSessionActive(sessionId)) return;
        state.mapRoulette.eliminatedIds.push(target.id);
        updateMapRouletteClasses();
        await delay(240);
  ...
  startDraft = async function rpmodsStartDraftV344() {
    if (state.startingDraft) return;
    state.startingDraft = true;
    setStartDraftLoading(true);
    await playUiSound(sounds.startDraft, 1);
    ...
    if (currentDraftConfig().phaseOrder === "map-first") {
      state.mapSelectionContext = "predraft";
      state.onlinePhase = currentRoomCode ? "map" : "local";
      startMapSelection({ predraft: true, force: true });
      scheduleDraftTimeout(() => {
        if (isDraftSessionActive() && !state.selectedMap && !state.mapRoulette.active) runMapRoulette({ predraft: true });
      }, 900);
      return;
    }
    state.mapSelectionContext = "postdraft";
  ...
  proceedAfterTurn = function rpmodsProceedAfterTurnV344() {
    if (!isDraftSessionActive()) return;
    if (state.turnIndex >= activeTurnCount()) {
      if (currentDraftConfig().phaseOrder === "map-first" && state.selectedMap) {
        showSummaryIntro();
      } else {
        state.mapSelectionContext = "postdraft";
        startMapSelection({ force: true });
      }
      return;
    }
    const nextTurn = currentTurn();
    const justEnteredBanPhase = state.turnIndex === 0 && nextTurn?.type === "ban";
    const justEnteredPickPhase = activeBanTurnCount() > 0 && state.turnIndex === activeBanTurnCount() && nextTurn?.type === "pick";
    if (justEnteredBanPhase || justEnteredPickPhase) {
      const isBan = nextTurn.type === "ban";
      showPhaseOverlay(
        isBan ? t("phase_ban") : t("phase_pick"),
        isBan ? systemDraftVoiceLines.voice_ban_phase.src : systemDraftVoiceLines.voice_pick_phase.src,
        isBan ? systemDraftVoiceLines.voice_ban_phase.text : systemDraftVoiceLines.voice_pick_phase.text,
  ...
  const previousStartOnlineDraftNow344 = startOnlineDraftNow;
  startOnlineDraftNow = async function rpmodsStartOnlineDraftNowV344(roomRef, roomData = {}) {
    const prepared = prepareOnlineDraftStart(roomData);
    const draftConfig = sanitizeDraftConfig({ ...prepared.draftConfig, mode: "advanced", pickPreset: "official-5v5" });
    if (draftConfig.phaseOrder !== "map-first") return previousStartOnlineDraftNow344(roomRef, { ...roomData, draftConfig });
    const { assignments, slots, players, captainA, captainB } = { ...prepared, draftConfig };
    resetDraftStateBeforeStart();
    state.onlineSlots = slots;
    state.players = players;
  ...
  function rp344Boot() {
    rp344EnsureConfigPanel();
    rp344ApplyChibiSize();
    state.rpmodsRequests = state.rpmodsRequests || {};
  }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", rp344Boot, { once: true });
  else rp344Boot();
})();

/* RPmods v3.4.5 — Pre-draft Map Isolation + Private Assist Protocol + Bot Simulation */
(function installRpmodsV345() {
  if (window.__rpmodsV345Installed) return;
  window.__rpmodsV345Installed = true;

  const VERSION = "3.4.5";
  const ASSIST_TIMEOUT_MS = 10000;
  const BOT_DECISION_MIN_MS = 850;
  const BOT_DECISION_MAX_MS = 1850;

  state.rp345PreDraftMapActive = false;
  state.rp345MapTimer = null;
  state.rp345MapToken = 0;
  state.rp345AssistTarget = null;
  state.rp345Assist = state.rp345Assist || { requests: {}, proposals: {} };
  state.rp345BotKeys = state.rp345BotKeys || {};

  function rp345Now() {
    try { return currentRoomCode ? onlineNow() : Date.now(); } catch (_) { return Date.now(); }
  }

  function rp345Uid(prefix = "evt") {
    return `${prefix}_${rp345Now()}_${Math.random().toString(36).slice(2, 9)}`;
  }

  function rp345Clone(value, fallback) {
    try { return JSON.parse(JSON.stringify(value)); } catch (_) { return fallback; }
  }

  function rp345NormalizeAssist(value = {}) {
    const raw = value && typeof value === "object" ? value : {};
    const requests = raw.requests && typeof raw.requests === "object" ? raw.requests : {};
    const proposals = raw.proposals && typeof raw.proposals === "object" ? raw.proposals : {};
    return { requests: { ...requests }, proposals: { ...proposals } };
  }

  function rp345TurnStage(index = state.turnIndex, config = currentDraftConfig()) {
    const turns = activeTurns(config);
    const turn = turns[index];
    if (!turn) return "complete";
    if (turn.type === "ban") return "bans";
    if (/^(captain|subcaptain)$/i.test(String(turn.slotKey || ""))) return "leader-picks";
    return "remaining-picks";
  }

  function rp345StageTitle(stage) {
    if (stage === "leader-picks") return "SELECCIÓN INICIAL DE LÍDERES";
    if (stage === "bans") return t("phase_ban");
    if (stage === "remaining-picks") return "SELECCIONES RESTANTES";
    return t("phase_pick");
  }

  function rp345StageVoice(stage) {
    return stage === "bans" ? systemDraftVoiceLines.voice_ban_phase : systemDraftVoiceLines.voice_pick_phase;
  }

  function rp345ShowStage(stage, callback = startTurn) {
    const voice = rp345StageVoice(stage);
    showPhaseOverlay(rp345StageTitle(stage), voice?.src || "", voice?.text || "", callback);
  }

  window.rpmodsDebugDraftOrder = function(config = currentDraftConfig()) {
    return activeTurns(config).map((turn, index) => ({
      index,
      stage: rp345TurnStage(index, config),
      type: turn.type,
      team: turn.team,
      slotKey: turn.slotKey || null,
      faction: turn.faction || null,
      groupId: turn.groupId ?? null,
      groupCount: turn.groupCount ?? 1,
    }));
  };

  /* The old extensions granted leaders direct control of any team turn. Restore
     strict ownership; delegation now always goes through a private proposal. */
  canControlCurrentTurn = function rpmodsCanControlCurrentTurnV345() {
    if (!currentRoomCode) return true;
    const pendingOwnProposal = Object.values(state.rp345Assist?.proposals || {}).some(item => item?.status === "pending" && item.initiatorClientId === onlineClientId() && item.turnIndex === state.turnIndex && item.draftSessionId === state.draftSessionId);
    if (pendingOwnProposal) return false;
    const turn = currentTurn();
    if (!turn || state.onlinePhase !== "draft" || !state.draftActive) return false;
    if (isAdvancedDraftConfig()) {
      const assigned = advancedSlotForTurn(turn);
      if (!assigned?.clientId || assigned.clientId !== onlineClientId()) return false;
    } else {
      if (currentRole !== "player" || currentOnlineTeamLetter() !== turn.team) return false;
    }
    if (state.turnStartedAt && onlineNow() < Number(state.turnStartedAt)) return false;
    return true;
  };

  function rp345CanInitiateDelegation() {
    if (!currentRoomCode || !isAdvancedDraftConfig() || !canControlCurrentTurn()) return false;
    const turn = currentTurn();
    if (!turn || turn.type !== "pick") return false;
    const own = advancedAssignmentForClient(onlineClientId(), state.onlineSlots);
    if (!own || own.team !== turn.team || own.slotKey !== turn.slotKey) return false;
    const mode = currentDraftConfig().delegationMode;
    if (mode === "none") return false;
    if (mode === "all_members") return true;
    if (mode === "captain_only") return own.slotKey === "captain";
  ...
  function rp345CanRequestPick() {
    if (!currentRoomCode || !isAdvancedDraftConfig() || state.onlinePhase !== "draft" || !state.draftActive) return false;
    const turn = currentTurn();
    if (!turn || turn.type !== "pick") return false;
    const own = advancedAssignmentForClient(onlineClientId(), state.onlineSlots);
    if (!own || own.team !== turn.team || own.slotKey === turn.slotKey) return false;
    if (!/^player[3-5]$/i.test(String(own.slotKey || ""))) return false;
    const ownIndex = advancedSlotIndex(own.slotKey);
    return !state.picks?.[own.team]?.[ownIndex];
  }

  function rp345CharacterForTeam(team, preferredName = "") {
    const valid = characters.filter(character => isCharacterAvailable(character, currentTurn()));
    if (!valid.length) return null;
    const preferred = valid.find(character => character.name === preferredName);
    if (preferred) return preferred;
    const pickedRoles = new Set((state.picks?.[team] || []).filter(Boolean).map(character => roleOf(character.name)));
    const rolePriority = ["Soporte", "Controlador", "Vanguardia", "Centinela", "Duelista"];
    const missing = rolePriority.find(role => !pickedRoles.has(role));
    const rolePool = missing ? valid.filter(character => roleOf(character.name) === missing) : [];
    const pool = rolePool.length ? rolePool : valid;
    return pool[Math.floor(Math.random() * pool.length)] || valid[0];
  }

  function rp345SlotByKey(team, slotKey) {
    return state.onlineSlots?.[team]?.[slotKey] || null;
  }

  function rp345IsBotSlot(team, slotKey) {
    const slot = rp345SlotByKey(team, slotKey);
    return Boolean(slot && isTestingBotParticipant(slot));
  }

  function rp345CurrentActor() {
    const turn = currentTurn();
    if (!turn) return null;
    if (!currentRoomCode) return { clientId: "local", name: state.players?.[turn.team]?.[turn.slotIndex] || "Jugador local", team: turn.team, slotKey: turn.slotKey };
    const slot = advancedSlotForTurn(turn);
    return slot ? { clientId: slot.clientId, name: slot.name, team: turn.team, slotKey: turn.slotKey, isBot: isTestingBotParticipant(slot) } : null;
  }

  function rp345WriteAssist(options = {}) {
    state.rp345Assist = rp345NormalizeAssist(state.rp345Assist);
    if (!currentRoomCode) {
      rp345RenderAssistUi();
      return;
    }
    const roomRef = roomRefFor(currentRoomCode);
    if (!roomRef) return;
    roomRef.child("draftState/rp345Assist").set(rp345Clone(state.rp345Assist, { requests: {}, proposals: {} })).catch(error => {
      console.warn("No se pudo sincronizar el protocolo privado de asignación.", error);
    });
    if (options.render !== false) rp345RenderAssistUi();
  }

  const previousOnlinePayload345 = currentOnlineDraftPayload;
  currentOnlineDraftPayload = function rpmodsOnlinePayloadV345(extra = {}) {
    return previousOnlinePayload345({ rp345Assist: rp345Clone(state.rp345Assist, { requests: {}, proposals: {} }), ...extra });
  };

  const previousBlankState345 = blankOnlineDraftState;
  blankOnlineDraftState = function rpmodsBlankStateV345(startPayload = {}) {
    return previousBlankState345({ rp345Assist: { requests: {}, proposals: {} }, preDraftMap: false, ...startPayload });
  };

  const previousApplyOnlineState345 = applyOnlineDraftState;
  applyOnlineDraftState = function rpmodsApplyOnlineStateV345(data = {}) {
    const draftState = data?.draftState || {};
    if (Object.prototype.hasOwnProperty.call(draftState, "rp345Assist")) {
      state.rp345Assist = rp345NormalizeAssist(draftState.rp345Assist);
    }
    const result = previousApplyOnlineState345(data);
    rp345ProcessAssistState();
    return result;
  };

  function rp345SwapAssignments(team, sourceSlotKey, targetSlotKey) {
    if (!team || !sourceSlotKey || !targetSlotKey || sourceSlotKey === targetSlotKey) return;
    if (currentRoomCode && isAdvancedDraftConfig()) {
      const slots = state.onlineSlots || emptyAdvancedSlots(activeTeamSize());
      const source = slots?.[team]?.[sourceSlotKey] || null;
      const target = slots?.[team]?.[targetSlotKey] || null;
      slots[team][sourceSlotKey] = target;
      slots[team][targetSlotKey] = source;
      state.onlineSlots = slots;
      applyAdvancedSlotsToPlayers(slots, currentDraftConfig());
      const roomRef = roomRefFor(currentRoomCode);
      roomRef?.update({
        slots: rp345Clone(slots, {}),
        "draftState/slots": rp345Clone(slots, {}),
        players: { A: [...state.players.A], B: [...state.players.B] },
        "draftState/players": { A: [...state.players.A], B: [...state.players.B] },
        updatedAt: onlineNow(),
      }).catch(error => console.warn("No se pudo sincronizar el intercambio de slots.", error));
      return;
    }
    const sourceIndex = advancedSlotIndex(sourceSlotKey);
    const targetIndex = advancedSlotIndex(targetSlotKey);
    const temp = state.players[team][sourceIndex];
    state.players[team][sourceIndex] = state.players[team][targetIndex];
    state.players[team][targetIndex] = temp;
  }

  function rp345CleanExpiredAssist() {
    const now = rp345Now();
    let changed = false;
    Object.entries(state.rp345Assist?.requests || {}).forEach(([id, item]) => {
      if (!item || item.status !== "pending" || Number(item.expiresAt || 0) <= now) {
        delete state.rp345Assist.requests[id];
        changed = true;
      }
    });
    Object.entries(state.rp345Assist?.proposals || {}).forEach(([id, item]) => {
      if (!item) {
        delete state.rp345Assist.proposals[id];
        changed = true;
        return;
      }
      ...
      if (["rejected", "expired", "completed"].includes(item.status) && now - Number(item.respondedAt || item.createdAt || now) > 2400) {
        delete state.rp345Assist.proposals[id];
        changed = true;
      }
    });
  ...
  function rp345CreateRequest(character) {
    if (!rp345CanRequestPick() || !character || !isCharacterAvailable(character, currentTurn())) return;
    const own = advancedAssignmentForClient(onlineClientId(), state.onlineSlots);
    const id = `${own.team}:${own.slotKey}`;
    state.rp345Assist.requests[id] = {
      id,
      status: "pending",
      team: own.team,
      slotKey: own.slotKey,
    ...
    state.preselectLocked = false;
    rp345WriteAssist();
    showAppNotice(`Solicitud enviada: ${character.name}.`, { type: "success", duration: 3500 });
    renderAll();
  }

  function rp345RejectRequest(requestId) {
    if (!state.rp345Assist?.requests?.[requestId]) return;
    delete state.rp345Assist.requests[requestId];
    rp345WriteAssist();
    showAppNotice("La solicitud de laminante fue rechazada.", { type: "warning" });
    renderAll();
  }

  function rp345AcceptRequest(requestId, options = {}) {
    const request = state.rp345Assist?.requests?.[requestId];
    const turn = currentTurn();
    if (!request || !turn || turn.type !== "pick" || turn.team !== request.team) return false;
    if (request.turnIndex !== state.turnIndex || request.draftSessionId !== state.draftSessionId) {
      delete state.rp345Assist.requests[requestId];
      rp345WriteAssist();
      return false;
    }
    ...
    if (!character || !isCharacterAvailable(character, turn)) {
      rp345RejectRequest(requestId);
      return false;
    }
    const actor = rp345CurrentActor();
    if (!options.botSystem && (!actor || actor.clientId !== onlineClientId())) return false;
    delete state.rp345Assist.requests[requestId];
    rp345SwapAssignments(turn.team, turn.slotKey, request.slotKey);
    state.selected = character;
    state.preselectLocked = true;
    rp345WriteAssist({ render: false });
    confirmTurn(false, { onlineSystem: Boolean(options.botSystem), rp345Delegated: true });
    return true;
  }

  function rp345CreateProposal(target, character, options = {}) {
    const turn = currentTurn();
    const actor = options.actor || rp345CurrentActor();
    if (!turn || turn.type !== "pick" || !target || !character || !actor) return null;
    const id = rp345Uid("assist");
    state.rp345Assist.proposals[id] = {
      id,
      status: "pending",
      team: turn.team,
      sourceSlotKey: turn.slotKey,
      targetSlotKey: target.slotKey,
      initiatorClientId: actor.clientId,
      initiatorName: actor.name || "Compañero",
      targetClientId: target.clientId,
      targetName: target.name || "Compañero",
      targetIsBot: Boolean(target.isBot),
      characterName: character.name,
      createdAt: rp345Now(),
      expiresAt: rp345Now() + ASSIST_TIMEOUT_MS,
      turnIndex: state.turnIndex,
      draftSessionId: state.draftSessionId,
    };
    state.selected = null;
    ...
    rp345WriteAssist();
    rp345RenderAssistUi();
    return state.rp345Assist.proposals[id];
  }

  function rp345RespondProposal(id, accepted) {
    const proposal = state.rp345Assist?.proposals?.[id];
    if (!proposal || proposal.status !== "pending") return;
    if (currentRoomCode && proposal.targetClientId !== onlineClientId()) return;
    proposal.status = accepted ? "accepted" : "rejected";
    proposal.respondedAt = rp345Now();
    proposal.respondedBy = currentRoomCode ? onlineClientId() : "local";
    rp345WriteAssist();
    rp345RemoveRecipientModal();
  }

  function rp345CompleteProposal(id) {
    const proposal = state.rp345Assist?.proposals?.[id];
    const turn = currentTurn();
    if (!proposal || proposal.status !== "accepted" || !turn) return false;
    if (proposal.turnIndex !== state.turnIndex || proposal.draftSessionId !== state.draftSessionId || proposal.team !== turn.team || proposal.sourceSlotKey !== turn.slotKey) {
    ...
    rp345SwapAssignments(turn.team, proposal.sourceSlotKey, proposal.targetSlotKey);
    proposal.status = "completed";
    proposal.respondedAt = rp345Now();
    state.selected = character;
    state.preselectLocked = true;
    rp345WriteAssist({ render: false });
    confirmTurn(false, { onlineSystem: true, rp345Delegated: true });
    return true;
  }

  function rp345RemoveRecipientModal() {
    document.getElementById("rp345-assist-recipient")?.remove();
  }

  function rp345ShowRecipientModal(proposal) {
    if (!proposal || proposal.status !== "pending") return;
    const existing = document.getElementById("rp345-assist-recipient");
    if (existing?.dataset.proposalId === proposal.id) return;
    existing?.remove();
    const character = characters.find(item => item.name === proposal.characterName);
    const modal = document.createElement("aside");
    modal.id = "rp345-assist-recipient";
    modal.className = "rp345-assist-notice rp345-assist-recipient";
    modal.dataset.proposalId = proposal.id;
    modal.innerHTML = `
      <div class="rp345-assist-avatar"><img src="${escapeHtml(character ? thumbPath(character.name) : "")}" alt=""></div>
      <div class="rp345-assist-message">
        <span>Solicitud de asignación</span>
        <strong>${escapeHtml(proposal.initiatorName)} está asegurándote ${escapeHtml(proposal.characterName)}</strong>
        <small>¿Deseas aceptar el laminante y cambiar el orden con tu compañero?</small>
        <div class="rp345-assist-progress"><i></i></div>
        <div class="rp345-assist-buttons"><button type="button" data-rp345-answer="accept">Aceptar</button><button type="button" data-rp345-answer="reject">Rechazar</button></div>
      </div>`;
    document.body.appendChild(modal);
    modal.querySelector('[data-rp345-answer="accept"]')?.addEventListener("click", () => rp345RespondProposal(proposal.id, true));
    modal.querySelector('[data-rp345-answer="reject"]')?.addEventListener("click", () => rp345RespondProposal(proposal.id, false));
    const remaining = Math.max(0, Number(proposal.expiresAt || 0) - rp345Now());
    modal.style.setProperty("--rp345-assist-duration", `${remaining}ms`);
    window.setTimeout(() => {
      const latest = state.rp345Assist?.proposals?.[proposal.id];
      if (latest?.status === "pending" && rp345Now() >= Number(latest.expiresAt || 0)) rp345RespondProposal(proposal.id, false);
    }, remaining + 80);
  }

  function rp345ShowWaitingNotice(proposal) {
    let notice = document.getElementById("rp345-assist-waiting");
    if (!proposal) {
      notice?.remove();
      return;
    }
    if (!notice) {
      ...
      notice.id = "rp345-assist-waiting";
      notice.className = "rp345-assist-notice rp345-assist-waiting";
      document.body.appendChild(notice);
    }
    notice.dataset.proposalId = proposal.id;
    notice.innerHTML = `<div class="rp345-waiting-dot"></div><div><span>Asignación enviada</span><strong>Esperando confirmación de ${escapeHtml(proposal.targetName)}...</strong><small>${escapeHtml(proposal.characterName)}</small></div>`;
  }

  function rp345ProcessAssistState() {
    state.rp345Assist = rp345NormalizeAssist(state.rp345Assist);
    const changed = rp345CleanExpiredAssist();
    const proposals = Object.values(state.rp345Assist.proposals || {});
    const me = currentRoomCode ? onlineClientId() : "local";
    const recipient = proposals.find(item => item.status === "pending" && item.targetClientId === me && !item.targetIsBot);
    if (recipient) rp345ShowRecipientModal(recipient);
    else rp345RemoveRecipientModal();
    const waiting = proposals.find(item => item.status === "pending" && item.initiatorClientId === me);
    rp345ShowWaitingNotice(waiting || null);

    if (currentRoomCode && currentRole === "host") {
      proposals.filter(item => item.status === "accepted").forEach(item => rp345CompleteProposal(item.id));
      proposals.filter(item => item.status === "pending" && item.targetIsBot).forEach(item => rp345ScheduleBotProposalResponse(item));
    } else if (!currentRoomCode) {
      proposals.filter(item => item.status === "accepted").forEach(item => rp345CompleteProposal(item.id));
    }
    if (changed && currentRoomCode && currentRole === "host") rp345WriteAssist({ render: false });
    rp345RenderRequestBubbles();
  }

  function rp345ScheduleBotProposalResponse(proposal) {
    const key = `proposal:${proposal.id}`;
    if (state.rp345BotKeys[key]) return;
    state.rp345BotKeys[key] = true;
    const delayMs = BOT_DECISION_MIN_MS + Math.round(Math.random() * (BOT_DECISION_MAX_MS - BOT_DECISION_MIN_MS));
    window.setTimeout(() => {
      const latest = state.rp345Assist?.proposals?.[proposal.id];
      if (!latest || latest.status !== "pending") return;
      latest.status = Math.random() < 0.88 ? "accepted" : "rejected";
      latest.respondedAt = rp345Now();
      latest.respondedBy = latest.targetClientId;
      rp345WriteAssist();
      if (latest.status === "accepted") rp345CompleteProposal(latest.id);
      else scheduleTestingBotTurn();
    }, delayMs);
  }

  function rp345RequestCanBeManaged(request) {
    const actor = rp345CurrentActor();
    if (!request || !actor || actor.team !== request.team) return false;
    if (actor.clientId === onlineClientId()) return canControlCurrentTurn();
    return Boolean(currentRole === "host" && actor.isBot);
  }

  function rp345RenderRequestBubbles() {
    document.querySelectorAll(".rp345-request-bubble").forEach(node => node.remove());
    const requests = Object.values(state.rp345Assist?.requests || {});
    requests.forEach(request => {
      if (request.status !== "pending" || request.turnIndex !== state.turnIndex || request.draftSessionId !== state.draftSessionId) return;
      const slot = document.querySelector(`.player-slot[data-team="${request.team}"][data-slot-index="${request.slotIndex}"]`)
        || document.querySelector(`.rpmods-assist-slot[data-team="${request.team}"][data-slot-index="${request.slotIndex}"]`);
      if (!slot) return;
      const bubble = document.createElement("div");
      bubble.className = "rp345-request-bubble";
      const manageable = rp345RequestCanBeManaged(request);
      bubble.innerHTML = `<span>${escapeHtml(request.requesterName)} quiere usar</span><strong>${escapeHtml(request.characterName)}</strong>${manageable ? '<div><button type="button" data-rp345-request="accept">Aceptar</button><button type="button" data-rp345-request="reject">Rechazar</button></div>' : ''}`;
      if (manageable) {
        bubble.querySelector('[data-rp345-request="accept"]')?.addEventListener("click", event => { event.stopPropagation(); rp345AcceptRequest(request.id, { botSystem: Boolean(rp345CurrentActor()?.isBot) }); });
        bubble.querySelector('[data-rp345-request="reject"]')?.addEventListener("click", event => { event.stopPropagation(); rp345RejectRequest(request.id); });
      }
      slot.appendChild(bubble);
    });
  }

  function rp345RenderAssistUi() {
    rp345ProcessAssistState();
    renderSelected();
  }

  function rp345TargetFromSlot(team, index) {
    const slotKey = ADVANCED_SLOT_KEYS[index];
    const slot = state.onlineSlots?.[team]?.[slotKey];
    if (!slotKey || !slot?.clientId) return null;
    return { team, slotKey, slotIndex: index, clientId: slot.clientId, name: slot.name || state.players?.[team]?.[index] || `Jugador ${index + 1}`, isBot: isTestingBotParticipant(slot) };
  }

  function rp345RemoveSlotMenu() {
    document.getElementById("rp345-slot-menu")?.remove();
  }

  function rp345ShowSlotMenu(slotElement, target) {
    rp345RemoveSlotMenu();
    const rect = slotElement.getBoundingClientRect();
    const menu = document.createElement("div");
    menu.id = "rp345-slot-menu";
    menu.className = "rp345-slot-menu";
    menu.innerHTML = `<button type="button" data-rp345-slot-action="select">Seleccionar PJ</button><button type="button" data-rp345-slot-action="cancel">Cancelar</button>`;
    document.body.appendChild(menu);
    const half = 112;
    const x = Math.max(half + 12, Math.min(window.innerWidth - half - 12, rect.left + rect.width / 2));
    const y = Math.max(88, rect.top - 10);
    menu.style.left = `${Math.round(x)}px`;
    menu.style.top = `${Math.round(y)}px`;
    menu.querySelector('[data-rp345-slot-action="select"]')?.addEventListener("click", () => {
      const turn = currentTurn();
      state.rp345AssistTarget = {
        ...target,
        sourceSlotKey: turn?.slotKey,
        turnIndex: state.turnIndex,
        draftSessionId: state.draftSessionId,
      ...
      rp345RemoveSlotMenu();
      showAppNotice(`Selecciona y confirma el laminante para ${target.name}.`, { type: "info", duration: 4200 });
      renderSelected();
    });
    menu.querySelector('[data-rp345-slot-action="cancel"]')?.addEventListener("click", rp345RemoveSlotMenu);
  }

  document.addEventListener("click", event => {
    const slot = event.target?.closest?.(".player-slot, .rpmods-assist-slot");
    if (!slot || event.target.closest("button") || !currentRoomCode) return;
    const team = slot.dataset.team;
    const index = Number(slot.dataset.slotIndex);
    const turn = currentTurn();
    if (!team || !Number.isFinite(index) || !turn || turn.type !== "pick" || turn.team !== team || !rp345CanInitiateDelegation()) return;
    const target = rp345TargetFromSlot(team, index);
    if (!target || target.slotKey === turn.slotKey || state.picks?.[team]?.[index]) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    rp345ShowSlotMenu(slot, target);
  }, true);

  document.addEventListener("click", event => {
    if (!event.target.closest("#rp345-slot-menu") && !event.target.closest(".player-slot, .rpmods-assist-slot")) rp345RemoveSlotMenu();
  }, true);

  const previousUpdateSlot345 = updatePlayerSlotElement;
  updatePlayerSlotElement = function rpmodsUpdatePlayerSlotV345(slot, team, index, turn) {
    previousUpdateSlot345(slot, team, index, turn);
    slot.dataset.team = team;
    slot.dataset.slotIndex = String(index);
    slot.querySelectorAll(".rpmods-request-bubble, .rpmods-slot-menu").forEach(node => node.remove());
    rp345RenderRequestBubbles();
  };

  function rp345EnsureRequestButton() {
    const anchor = document.getElementById("confirm-action");
    if (!anchor) return null;
    const old = document.getElementById("rpmods-request-pick-action");
    let button = old;
    if (!button || button.dataset.rp345Bound !== "1") {
      const replacement = document.createElement("button");
      replacement.id = "rpmods-request-pick-action";
      replacement.type = "button";
      replacement.className = "random-selection-button rpmods-request-pick-action hidden";
      replacement.dataset.rp345Bound = "1";
      replacement.textContent = "PEDIR";
      replacement.addEventListener("click", event => {
        event.preventDefault();
        event.stopImmediatePropagation();
        if (!rp345CanRequestPick()) return;
        if (!state.selected || !isCharacterAvailable(state.selected, currentTurn())) {
          showAppNotice("Selecciona primero el laminante que deseas pedir.", { type: "warning" });
          return;
        }
        rp345CreateRequest(state.selected);
      });
      if (old) old.replaceWith(replacement);
      else anchor.insertAdjacentElement("afterend", replacement);
      button = replacement;
    }
    return button;
  }

  const previousRenderSelected345 = renderSelected;
  renderSelected = function rpmodsRenderSelectedV345() {
    previousRenderSelected345();
    const confirm = document.getElementById("confirm-action");
    const request = rp345EnsureRequestButton();
    const canRequest = rp345CanRequestPick();
    if (request) {
      request.classList.toggle("hidden", !canRequest);
      request.disabled = !state.selected || !isCharacterAvailable(state.selected, currentTurn()) || state.locked;
      request.textContent = state.selected ? `PEDIR ${state.selected.name}` : "PEDIR";
    }
    if (confirm) {
      const waitingOwnProposal = currentRoomCode && Object.values(state.rp345Assist?.proposals || {}).some(item => item?.status === "pending" && item.initiatorClientId === onlineClientId() && item.turnIndex === state.turnIndex && item.draftSessionId === state.draftSessionId);
      if (canRequest || waitingOwnProposal) confirm.classList.add("hidden");
      else if (state.rp345AssistTarget) {
        confirm.classList.remove("hidden");
        confirm.textContent = "ENVIAR SOLICITUD";
      }
    }
    rp345RenderRequestBubbles();
  };

  const previousConfirmTurn345 = confirmTurn;
  confirmTurn = function rpmodsConfirmTurnV345(isAuto = false, options = {}) {
    if (state.rp345AssistTarget && !options.rp345Delegated) {
      const target = state.rp345AssistTarget;
      const turn = currentTurn();
      if (!turn || !canControlCurrentTurn() || target.turnIndex !== state.turnIndex || target.draftSessionId !== state.draftSessionId) {
        state.rp345AssistTarget = null;
        showAppNotice("La oportunidad de asignación ya no está disponible.", { type: "warning" });
        return;
      }
      if (!state.selected || !isCharacterAvailable(state.selected, turn)) {
        showAppNotice("Selecciona el laminante que quieres asegurar.", { type: "warning" });
        return;
      }
      const character = state.selected;
      state.rp345AssistTarget = null;
      rp345CreateProposal(target, character);
      renderAll();
      return;
    }

    const allowOnlineSystem = Boolean(options.onlineSystem);
    if (!state.draftActive || (!allowOnlineSystem && !canControlCurrentTurn())) return;
    const turn = currentTurn();
    ...
    timerCore?.classList.remove("timer-warning");
    const confirmedCharacter = state.selected;
    const previousStage = rp345TurnStage(state.turnIndex);
    let wait = 360;
    let actionEvent = null;
    ...
    if (turn.type === "ban") {
      ...
      state.flashBan = confirmedCharacter.name;
      state.banAnimation = { character: confirmedCharacter, team: turn.team };
      actionEvent = createOnlineActionEvent("ban", turn.team, confirmedCharacter, { isAuto, delegated: Boolean(options.rp345Delegated) });
      audioPlay(sounds.ban, 0.86, "sfx");
      playCharacterVoice(confirmedCharacter, "ban");
      wait = confirmedActionAnimationDuration("ban", isAuto) + 150;
    } else {
      ...
      actionEvent = createOnlineActionEvent("pick", turn.team, confirmedCharacter, { isAuto, delegated: Boolean(options.rp345Delegated) });
      audioPlay(sounds.confirm, 0.86, "sfx");
      playCharacterVoice(confirmedCharacter, "pick");
    }
    ...
    syncDraftAnimationTiming();
    renderConfirmedActionState();
    if (currentRoomCode) pushOnlineDraftState({ actionEvent, phase: "draft" });
    ...
    scheduleDraftTimeout(() => {
      ...
      const completed = state.turnIndex >= activeTurnCount();
      const nextStage = completed ? "complete" : rp345TurnStage(state.turnIndex);
      if (currentRoomCode) {
        if (completed) {
          if (currentDraftConfig().phaseOrder === "map-first" && state.selectedMap) {
            state.onlinePhase = "summary";
            state.turnStartedAt = null;
            state.turnDeadlineAt = null;
            pushOnlineDraftState({ phase: "summary", actionEvent: null, audioEvent: createOnlineAudioEvent("finishDraft", { playForOrigin: false }) });
          } else {
            state.onlinePhase = "map";
            state.turnStartedAt = null;
            state.turnDeadlineAt = null;
            pushOnlineDraftState({ phase: "map", actionEvent: null, audioEvent: createOnlineAudioEvent("mapSelector") });
          }
        } else {
          const stageChanged = previousStage !== nextStage;
          prepareClockForTurnIndex(state.turnIndex, stageChanged ? phaseOverlayDurationMs() : 0);
          state.onlinePhase = "draft";
          const phaseEvent = stageChanged ? createOnlinePhaseEvent(`${nextStage}:${state.turnIndex}`) : null;
          if (phaseEvent) onlineLastPhaseEventId = phaseEvent.id;
          const audioEvent = stageChanged ? null : createOnlineAudioEvent("turnNarration", { turnIndex: state.turnIndex });
          pushOnlineDraftState({ phase: "draft", actionEvent: null, phaseEvent, audioEvent });
        }
      }
      proceedAfterTurn();
  ...
  /* ---------------- Pre-draft map: isolated from the turn engine ---------------- */
  function rp345MapSessionAlive(sessionId) {
    return state.draftSessionId === sessionId && (state.rp345PreDraftMapActive || state.draftActive);
  }

  function rp345ClearMapTimer() {
    if (state.rp345MapTimer) window.clearTimeout(state.rp345MapTimer);
    state.rp345MapTimer = null;
  }

  function rp345EnterMapScreen(options = {}) {
    const predraft = Boolean(options.predraft);
    rp345ClearMapTimer();
    clearInterval(state.timerId);
    state.rp345PreDraftMapActive = predraft;
    state.mapSelectionContext = predraft ? "predraft" : "postdraft";
    state.onlinePhase = "map";
    state.turnStartedAt = null;
    state.turnDeadlineAt = null;
    state.locked = true;
    ...
    state.pickAnimation = null;
    state.roulette = { active: false, highlightedName: null, finalName: null, previewCharacter: null };
    state.mapRoulette = { active: false, highlightedId: null, finalId: null, eliminatedIds: [] };
    switchScreen(mapScreen);
    mapScreen?.classList.remove("map-enter");
    void mapScreen?.offsetWidth;
    mapScreen?.classList.add("map-enter");
    ...
    if (canRun && !options.fromOnline) {
      const sessionId = state.draftSessionId;
      const token = ++state.rp345MapToken;
      state.rp345MapTimer = window.setTimeout(() => {
        if (token !== state.rp345MapToken || !rp345MapSessionAlive(sessionId) || state.selectedMap || state.mapRoulette.active) return;
        runMapRoulette({ predraft });
      }, 900);
    }
  }

  startMapSelection = function rpmodsStartMapSelectionV345(options = {}) {
    const predraft = Boolean(options.predraft || state.rp345PreDraftMapActive || (currentDraftConfig().phaseOrder === "map-first" && !state.selectedMap && state.turnIndex === 0));
    if (predraft && !state.rp345PreDraftMapActive && !options.force) return;
    if (!predraft && !state.draftActive) return;
    rp345EnterMapScreen({ ...options, predraft });
    if (currentRoomCode && currentRole === "host" && !options.fromOnline) {
      pushOnlineDraftPatch({ phase: "map", preDraftMap: predraft, force: true, turnStartedAt: null, turnDeadlineAt: null });
    }
  };

  function rp345VoiceCandidates() {
    const values = Array.from({ length: 8 }, (_, index) => `audio/map_vote_chibi_${index + 1}.ogg`);
  ...
  function rp345PlayChibiVoice() {
    unlockMediaPlayback(true);
    audioPlayFromSourceList(rp345VoiceCandidates(), 1, "sfx");
  }

  function rp345EnsureChibi() {
    let overlay = document.getElementById("map-chibi-overlay-v345");
    if (overlay) return overlay;
    overlay = document.createElement("div");
    overlay.id = "map-chibi-overlay-v345";
    overlay.className = "map-chibi-overlay-v345 hidden";
    overlay.innerHTML = `<img class="map-chibi-fall-v345" alt="chibi" src="img/ui/map_chibi_fall.png"><img class="map-chibi-land-v345" alt="chibi" src="img/ui/map_chibi_land.png">`;
    document.body.appendChild(overlay);
    return overlay;
  }

  async function rp345DropChibi(mapId, finalPause = false) {
    const card = mapGrid?.querySelector(`.map-card[data-map-id="${mapId}"]`);
    if (!card) return;
    if (finalPause) await delay(2000);
    const overlay = rp345EnsureChibi();
    const rect = card.getBoundingClientRect();
    overlay.style.left = `${Math.round(rect.left + rect.width / 2)}px`;
    overlay.style.top = `${Math.round(rect.top + rect.height / 2 - 30)}px`;
    overlay.classList.remove("hidden", "is-dropping", "is-landing");
    void overlay.offsetWidth;
    overlay.classList.add("is-dropping");
    await delay(540);
    overlay.classList.remove("is-dropping");
  ...
  function rp345EliminationPlan(total) {
    if (total <= 1) return [];
    if (total === 9) return [3, 2, 2, 1];
    const plan = [];
    let remaining = total - 1;
    while (remaining > 0) {
      const count = !plan.length && remaining >= 3 ? 3 : remaining >= 2 ? 2 : 1;
      plan.push(count);
      remaining -= count;
    }
    return plan;
  ...
  async function rp345PushMapEvent(event, extra = {}) {
    if (!currentRoomCode || currentRole !== "host") return;
    pushOnlineDraftPatch({ phase: "map", preDraftMap: state.rp345PreDraftMapActive, force: true, mapEvent: event, mapRoulette: serializeMapRouletteForOnline(state.mapRoulette), ...extra });
  }

  async function rp345FinishMap(selected) {
    state.selectedMap = selected;
    state.mapRoulette.highlightedId = selected.id;
    state.mapRoulette.finalId = selected.id;
    state.mapRoulette.active = false;
    ...
    updateSelectedMapCopy();
    if (currentRoomCode && currentRole === "host") {
      await rp345PushMapEvent({ id: rp345Uid("mapSelected"), type: "mapSelected", mapId: selected.id, byClientId: onlineClientId() }, { selectedMap: { id: selected.id, name: selected.name } });
    }
    await delay(420);
    if (state.rp345PreDraftMapActive) rp345ActivateDraftAfterMap();
    else showSummaryIntro();
  }

  runMapRoulette = async function rpmodsRunMapRouletteV345(options = {}) {
    const sessionId = state.draftSessionId;
    if (!rp345MapSessionAlive(sessionId) || state.mapRoulette.active || !maps.length) return;
    state.mapRoulette.active = true;
    state.mapRoulette.finalId = null;
    state.mapRoulette.highlightedId = null;
    state.mapRoulette.eliminatedIds = [];
    ...
    if (config.mapRandomMode === "classic-random") {
      ...
      for (let index = 0; index < 20; index += 1) {
        ...
        updateMapRouletteClasses();
        audioPlay(sounds.mapRoulette || sounds.roulette, 0.72, "sfx");
        await delay(62 + index * 9);
        if (!rp345MapSessionAlive(sessionId)) return;
      }
      await rp345FinishMap(selected);
      return;
    }

    const pool = [...maps];
    const plan = rp345EliminationPlan(pool.length);
    for (let groupIndex = 0; groupIndex < plan.length; groupIndex += 1) {
      for (let itemIndex = 0; itemIndex < plan[groupIndex]; itemIndex += 1) {
        if (pool.length <= 1 || !rp345MapSessionAlive(sessionId)) return;
        const target = pool.splice(Math.floor(Math.random() * pool.length), 1)[0];
        state.mapRoulette.highlightedId = target.id;
        updateMapRouletteClasses();
        const finalPause = groupIndex === plan.length - 1 && itemIndex === plan[groupIndex] - 1;
        if (currentRoomCode && currentRole === "host") {
          rp345PushMapEvent({ id: rp345Uid("chibiDrop"), type: "chibiDrop", mapId: target.id, finalPause, byClientId: onlineClientId() });
        }
        await rp345DropChibi(target.id, finalPause);
        state.mapRoulette.eliminatedIds.push(target.id);
        updateMapRouletteClasses();
        if (currentRoomCode && currentRole === "host") {
          rp345PushMapEvent({ id: rp345Uid("mapEliminated"), type: "mapEliminated", mapId: target.id, byClientId: onlineClientId() });
        }
        await delay(240);
      }
    }
    await rp345FinishMap(pool[0] || randomFrom(maps));
  };

  const previousHandleMapEvent345 = handleOnlineMapEvent;
  handleOnlineMapEvent = function rpmodsHandleMapEventV345(event) {
    if (event?.type === "chibiDrop" && event.byClientId !== onlineClientId()) {
      if (!event.id || event.id === onlineLastMapEventId) return;
      onlineLastMapEventId = event.id;
      rp345DropChibi(event.mapId, Boolean(event.finalPause));
      return;
    }
    if (event?.type === "mapEliminated" && event.byClientId !== onlineClientId()) {
      if (!event.id || event.id === onlineLastMapEventId) return;
      onlineLastMapEventId = event.id;
      state.mapRoulette.eliminatedIds = Array.from(new Set([...(state.mapRoulette.eliminatedIds || []), event.mapId]));
      updateMapRouletteClasses();
      return;
    }
    return previousHandleMapEvent345(event);
  };

  window.rpFinishMapSelectionContext = function rpmodsFinishMapSelectionContextV345() {
    if (state.rp345PreDraftMapActive) rp345ActivateDraftAfterMap();
    else showSummaryIntro();
  };

  function rp345ActivateDraftAfterMap() {
    ...
    if (currentRoomCode && currentRole === "host") {
      pushOnlineDraftState({
        ...
        phaseEvent: createOnlinePhaseEvent("leaderPickPhase"),
        force: true,
      });
    }
    switchScreen(draftScreen);
    setupBackgroundVideo();
    startMusic("draft");
    rp345ShowStage("leader-picks", startTurn);
  }

  startDraft = async function rpmodsStartDraftV345() {
    if (state.startingDraft) return;
    state.startingDraft = true;
    setStartDraftLoading(true);
    await playUiSound(sounds.startDraft, 1);
    ...
    rp345ClearMapTimer();
    state.draftSessionId += 1;
    state.rp345Assist = { requests: {}, proposals: {} };
    state.rpmodsRequests = {};
    state.startingDraft = false;
    setStartDraftLoading(false);
    setupBackgroundVideo();
    ...
    if (currentDraftConfig().phaseOrder === "map-first") {
      state.draftActive = false;
      state.rp345PreDraftMapActive = true;
      rp345EnterMapScreen({ predraft: true });
      return;
    }
    state.rp345PreDraftMapActive = false;
    state.draftActive = true;
    ...
    switchScreen(draftScreen);
    rp345ShowStage(rp345TurnStage(0), startTurn);
  };

  proceedAfterTurn = function rpmodsProceedAfterTurnV345() {
    if (!state.draftActive) return;
    if (state.turnIndex >= activeTurnCount()) {
      if (currentDraftConfig().phaseOrder === "map-first" && state.selectedMap) showSummaryIntro();
      else rp345EnterMapScreen({ predraft: false });
      return;
    }
    const previousStage = rp345TurnStage(Math.max(0, state.turnIndex - 1));
    const nextStage = rp345TurnStage(state.turnIndex);
    if (previousStage !== nextStage) rp345ShowStage(nextStage, startTurn);
    else startTurn();
  };

  const previousRenderTurnInfo345 = renderTurnInfo;
  renderTurnInfo = function rpmodsRenderTurnInfoV345() {
    previousRenderTurnInfo345();
    const turn = currentTurn();
    if (!turn) return;
    if (turn.type === "ban") {
      const banTurns = activeTurns().filter(item => item.type === "ban");
      const current = Math.max(1, banTurns.findIndex(item => item === turn) + 1);
      const roundText = t("ban_round", { current, total: banTurns.length });
      const batch = document.getElementById("batch-indicator");
      const status = document.getElementById("status-phase");
      if (batch) batch.textContent = isAdvancedDraftConfig() && turn.slotKey ? `${roundText} · ${advancedSlotLabel(turn.slotKey)}` : roundText;
      if (status) status.textContent = `${t("phase_ban")} · ${roundText}`;
    }
  };

  document.addEventListener("click", event => {
    const button = event.target?.closest?.("#randomize-map");
    if (!button || !state.rp345PreDraftMapActive) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    if (state.mapRoulette.active || state.selectedMap) return;
    runMapRoulette({ predraft: true });
  }, true);

  const previousSerializeMap345 = serializeMapRouletteForOnline;
  serializeMapRouletteForOnline = function rpmodsSerializeMapV345(value = {}) {
    return { ...previousSerializeMap345(value), eliminatedIds: Array.from(value.eliminatedIds || []) };
  };

  /* ---------------- Online map-first lifecycle ---------------- */
  const previousStartOnlineNow345 = startOnlineDraftNow;
  startOnlineDraftNow = async function rpmodsStartOnlineNowV345(roomRef, roomData = {}) {
    const prepared = prepareOnlineDraftStart(roomData);
    const draftConfig = sanitizeDraftConfig({ ...prepared.draftConfig, mode: "advanced", pickPreset: "official-5v5" });
    if (draftConfig.phaseOrder !== "map-first") return previousStartOnlineNow345(roomRef, { ...roomData, draftConfig });
    ...
    const payload = blankOnlineDraftState({
      ...
      turnStartedAt: null,
      turnDeadlineAt: null,
      rp345Assist: { requests: {}, proposals: {} },
    });
    await roomRef.update({
      started: true,
      startedAt: onlineNow(),
  ...
  startOnlineDraftFromRoom = function rpmodsStartOnlineFromRoomV345(data = {}) {
    if (!currentRoomCode) return;
    onlineLatestRoomData = data || {};
    const previousIndex345 = state.turnIndex;
    const previousStage345 = rp345TurnStage(previousIndex345);
    const draftState = data.draftState || {};
    ...
    const sessionKey = `${currentRoomCode}:${draftState.draftSessionId || 0}`;
    const firstForSession = state.rp345OnlineSessionKey !== sessionKey;
    if (firstForSession) {
      state.rp345OnlineSessionKey = sessionKey;
      resetDraftStateBeforeStart();
      clearDraftTimeouts();
    ...
    suppressOnlinePush = true;
    try { syncDraftStateFromRoom(data); } finally { suppressOnlinePush = false; }
    if (Array.isArray(draftState.mapRoulette?.eliminatedIds)) {
      state.mapRoulette.eliminatedIds = [...draftState.mapRoulette.eliminatedIds];
    }
    const currentStage345 = rp345TurnStage(state.turnIndex);
    const stageChanged345 = previousIndex345 !== state.turnIndex && previousStage345 !== currentStage345;
    onlineStartedForRoom = currentRoomCode;
    onlineRoomStartedState = true;
    setupBackgroundVideo();
    startMusic("draft");

    if (phase === "map" && config.phaseOrder === "map-first" && !state.selectedMap) {
      state.draftActive = false;
      state.rp345PreDraftMapActive = true;
      state.onlinePhase = "map";
      state.turnStartedAt = null;
      state.turnDeadlineAt = null;
      if (!hasScreenActive(mapScreen)) rp345EnterMapScreen({ predraft: true, fromOnline: currentRole !== "host", silentNarration: !firstForSession });
      else { renderMapGrid(); updateSelectedMapCopy(); }
      if (currentRole === "host" && !state.mapRoulette.active && !state.selectedMap) {
        const token = `${sessionKey}:host-map`;
        if (state.rp345HostMapToken !== token) {
          state.rp345HostMapToken = token;
          state.rp345MapTimer = window.setTimeout(() => {
            if (state.rp345PreDraftMapActive && !state.selectedMap && !state.mapRoulette.active) runMapRoulette({ predraft: true });
          }, 900);
        }
      }
      return;
    ...
    if (phase === "draft") {
      const entering = !state.draftActive || !hasScreenActive(draftScreen);
      state.rp345PreDraftMapActive = false;
      state.draftActive = true;
      state.onlinePhase = "draft";
      switchScreen(draftScreen);
      if (entering) {
        const stage = rp345TurnStage(state.turnIndex);
        if (firstForSession || state.turnIndex === 0 || stageChanged345) rp345ShowStage(stage, () => startTurn({ skipNarration: true }));
        else startTurn({ skipNarration: true });
      } else if (stageChanged345) {
        rp345ShowStage(currentStage345, () => startTurn({ skipNarration: true }));
      } else if (!state.locked && !state.roulette.active && state.turnDeadlineAt) {
        updateTimerDisplay(onlineRemainingSeconds());
      }
      rp345ProcessAssistState();
      scheduleTestingBotTurn();
  ...
  /* ---------------- Bot participation in requests and delegation ---------------- */
  function rp345BotRequestKey(turn, slotKey) {
    return `botRequest:${state.draftSessionId}:${state.turnIndex}:${turn.team}:${slotKey}`;
  }

  function rp345MaybeCreateBotMemberRequest() {
    if (!currentRoomCode || currentRole !== "host" || !state.draftActive || state.onlinePhase !== "draft") return;
    const turn = currentTurn();
    if (!turn || turn.type !== "pick" || rp345TurnStage() !== "leader-picks") return;
    const candidates = ["player3", "player4", "player5"].filter(slotKey => {
      const slot = state.onlineSlots?.[turn.team]?.[slotKey];
      const index = advancedSlotIndex(slotKey);
      return slot && isTestingBotParticipant(slot) && !state.picks?.[turn.team]?.[index] && !state.rp345Assist.requests?.[`${turn.team}:${slotKey}`];
    });
    candidates.forEach(slotKey => {
      const key = rp345BotRequestKey(turn, slotKey);
      if (state.rp345BotKeys[key] || Math.random() > 0.38) return;
      state.rp345BotKeys[key] = true;
      window.setTimeout(() => {
        if (!state.draftActive || state.turnIndex !== Number(key.split(":")[2])) return;
        const slot = state.onlineSlots?.[turn.team]?.[slotKey];
        const character = rp345CharacterForTeam(turn.team);
        if (!slot || !character) return;
        const id = `${turn.team}:${slotKey}`;
        state.rp345Assist.requests[id] = {
          id,
          status: "pending",
          team: turn.team,
          slotKey,
          slotIndex: advancedSlotIndex(slotKey),
          requesterClientId: slot.clientId,
          requesterName: slot.name,
          characterName: character.name,
          createdAt: rp345Now(),
          expiresAt: rp345Now() + 30000,
          turnIndex: state.turnIndex,
  ...
  function rp345MaybeHandleBotTurn() {
    if (!currentRoomCode || currentRole !== "host" || !state.draftActive || state.onlinePhase !== "draft") return false;
    const turn = currentTurn();
    const botId = botClientIdForCurrentTurn(onlineLatestRoomData || {}, turn);
    if (!turn || !botId) return false;
    const key = `botTurnAssist:${state.draftSessionId}:${state.turnIndex}:${botId}`;
    if (state.rp345BotKeys[key]) return true;

    const requests = Object.values(state.rp345Assist?.requests || {}).filter(item => item.status === "pending" && item.team === turn.team && item.turnIndex === state.turnIndex && item.draftSessionId === state.draftSessionId);
    if (requests.length) {
      state.rp345BotKeys[key] = true;
      const request = requests[0];
      window.setTimeout(() => {
        if (Math.random() < 0.82) rp345AcceptRequest(request.id, { botSystem: true });
        else {
          rp345RejectRequest(request.id);
          delete state.rp345BotKeys[key];
          scheduleTestingBotTurn();
        }
      }, BOT_DECISION_MIN_MS + Math.round(Math.random() * 700));
      return true;
    }

    const sourceSlot = state.onlineSlots?.[turn.team]?.[turn.slotKey];
    const delegationAllowed = sourceSlot && isTestingBotParticipant(sourceSlot) && ["all_members", "captain_only", "captain_subcaptain"].includes(currentDraftConfig().delegationMode)
      && (currentDraftConfig().delegationMode === "all_members" || turn.slotKey === "captain" || (currentDraftConfig().delegationMode === "captain_subcaptain" && turn.slotKey === "subcaptain"));
    if (delegationAllowed && Math.random() < 0.28) {
      const targetKeys = advancedSlotsForTeamSize(activeTeamSize()).filter(slotKey => slotKey !== turn.slotKey && !state.picks?.[turn.team]?.[advancedSlotIndex(slotKey)] && state.onlineSlots?.[turn.team]?.[slotKey]?.clientId);
      const targetKey = targetKeys[Math.floor(Math.random() * targetKeys.length)];
      const character = rp345CharacterForTeam(turn.team);
      if (targetKey && character) {
        state.rp345BotKeys[key] = true;
        const targetSlot = state.onlineSlots[turn.team][targetKey];
        rp345CreateProposal({ team: turn.team, slotKey: targetKey, slotIndex: advancedSlotIndex(targetKey), clientId: targetSlot.clientId, name: targetSlot.name, isBot: isTestingBotParticipant(targetSlot) }, character, { actor: { clientId: sourceSlot.clientId, name: sourceSlot.name, team: turn.team, slotKey: turn.slotKey, isBot: true } });
        return true;
      }
    }
    return false;
  ...
  const previousScheduleBot345 = scheduleTestingBotTurn;
  scheduleTestingBotTurn = function rpmodsScheduleBotV345() {
    rp345MaybeCreateBotMemberRequest();
    rp345ProcessAssistState();
    if (rp345MaybeHandleBotTurn()) return;
    return previousScheduleBot345();
  };

  /* Clean old UI and run maintenance. */
  function rp345Maintenance() {
    document.querySelectorAll(".rpmods-assist-confirm, .rpmods-slot-menu, .rpmods-request-bubble").forEach(node => node.remove());
    state.rpmodsAssistTarget = null;
    state.rpmodsRequests = {};
    rp345ProcessAssistState();
  }
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", rp345Maintenance, { once: true });
  else rp345Maintenance();
})();
