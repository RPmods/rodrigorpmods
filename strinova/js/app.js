...
function applyLanguage(lang = currentLanguage(), options = {}) {
  ...
  document.querySelectorAll('.setup-top-tab').forEach(button=>{ const key={menu:'tab_menu',tournament:'tab_tournament',gacha:'tab_gacha',volumen:'tab_volume',configuracion:'tab_config',random:'tab_random',idioma:'tab_language',updates:'tab_updates',creditos:'tab_credits'}[button.dataset.tab]; if(key) button.textContent=t(key); });
...
function activateSetupTab(tabName) {
  ...
  if (setupShell) {
    setupShell.classList.remove("view-menu", "view-tournament", "view-gacha", "view-volumen", "view-configuracion", "view-development", "view-random", "view-idioma", "view-creditos", "view-updates");
