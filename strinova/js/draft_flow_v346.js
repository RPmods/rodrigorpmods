...
(function installDraftFlowV346() {
  ...
  function buildRemainingTurns(size) {
    // v3.4.18: se elimina definitivamente el doble pick simultáneo.
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — Map reveal, selector gating and safer bot flow
   * ---------------------------------------------------------------- */
  ...
  runMapRoulette = async function runMapRouletteV348(options = {}) {
    ...
    for (let groupIndex = 0; groupIndex < plan.length; groupIndex += 1) {
      for (let index = 0; index < plan[groupIndex]; index += 1) {
        ...
        // v3.4.18: do not illuminate the card before impact.
        state.mapRoulette.highlightedId = null;
        updateMapRouletteClasses();
  ...
  // v3.4.18: bots keep the draft moving, but no longer create random teammate proposals.
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — Turn ownership, teammate menu and bot guardrails
   * ---------------------------------------------------------------- */
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — Hard recovery for selector and teammate slot actions
   * ---------------------------------------------------------------- */
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — Ownership isolation, assist swap fix and anti-stall watchdog
   * ---------------------------------------------------------------- */
  function v3411TurnKeys(turn = currentTurn()) {
    return (turn?.slotKeys || [turn?.slotKey]).filter(Boolean);
  }

  function v3411OwnAssignment() {
    return currentRoomCode ? advancedAssignmentForClient(onlineClientId(), state.onlineSlots) : null;
  }

  function v3411OwnsCurrentTurn() {
    if (flow.phase !== "draft" || !state.draftActive) return false;
    const turn = currentTurn();
    if (!turn) return false;
    if (!currentRoomCode) return true;
    ...
    if (isAdvancedDraftConfig()) {
      const own = v3411OwnAssignment();
      if (!own || own.team !== turn.team) return false;
      if (turn.type === "pick" && turn.simultaneous && simultaneousRecord(turn)[own.slotKey]) return false;
      return v3411TurnKeys(turn).includes(own.slotKey);
    }
    return currentOnlineTeamLetter() === turn.team;
  }

  canControlCurrentTurn = function canControlCurrentTurnV3411() {
    return v3411OwnsCurrentTurn();
  };

  canDelegate = function canDelegateV3411() {
    if (!v3411OwnsCurrentTurn() || !isAdvancedDraftConfig()) return false;
    const turn = currentTurn();
    const own = v3411OwnAssignment();
    if (!turn || turn.type !== "pick" || !own || own.team !== turn.team) return false;
    const mode = currentDraftConfig().delegationMode;
  ...
  function v3411ShouldShowSelector() {
    if (flow.phase !== "draft" || !state.draftActive || state.locked || state.roulette.active) return false;
    return Boolean(v3411OwnsCurrentTurn() || canRequest() || flow.assistTarget);
  }

  shouldShowSelectorV348 = v3411ShouldShowSelector;
  syncSelectorVisibilityV348 = function syncSelectorVisibilityV3411() {
    const show = v3411ShouldShowSelector();
    document.body.classList.toggle("rp348-selector-hidden", !show);
    document.body.classList.toggle("rp3410-selector-force-visible", show);
    document.body.classList.toggle("rp3411-selector-owned", show);
    if (!show && !flow.assistTarget) {
      state.selected = null;
      state.preselectLocked = false;
    }
    if (characterGrid) {
  ...
  function v3411TeamFromSlotElement(element) {
    if (element?.closest?.("#team-a-slots")) return "A";
    if (element?.closest?.("#team-b-slots")) return "B";
    return "";
  }

  function v3411SlotKeyFromElement(element) {
    const parent = element?.parentElement;
    if (!parent) return null;
    const index = Array.from(parent.children || []).indexOf(element);
    return advancedSlotsForTeamSize(activeTeamSize())[index] || null;
  ...
  function v3411SlotCanReceiveDelegation(team, slotKey) {
    const turn = currentTurn();
    if (!canDelegate() || !turn || turn.type !== "pick" || team !== turn.team || !slotKey) return false;
    if (v3411TurnKeys(turn).includes(slotKey)) return false;
    const slot = state.onlineSlots?.[team]?.[slotKey];
    if (!slot?.clientId) return false;
    return !state.picks?.[team]?.[advancedSlotIndex(slotKey)];
  }

  function v3411BindDelegatableSlots() {
    document.querySelectorAll("#team-a-slots > *, #team-b-slots > *").forEach(element => {
      const team = v3411TeamFromSlotElement(element);
      const slotKey = v3411SlotKeyFromElement(element);
      const valid = v3411SlotCanReceiveDelegation(team, slotKey);
      element.classList.toggle("rp346-delegatable-slot", valid);
      if (valid) {
        element.dataset.rp346TargetTeam = team;
        element.dataset.rp346TargetSlot = slotKey;
      } else {
  ...
  bindSlotMenus = function bindSlotMenusV3411() {
    v3411BindDelegatableSlots();
  };
  ...
  document.addEventListener("click", event => {
    ...
    const team = v3411TeamFromSlotElement(element);
    const slotKey = v3411SlotKeyFromElement(element);
    if (!v3411SlotCanReceiveDelegation(team, slotKey)) return;
    const slot = state.onlineSlots?.[team]?.[slotKey];
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
  ...
  function v3411CurrentActorSlotKey(turn = currentTurn(), options = {}) {
    if (options.botSlotKey) return options.botSlotKey;
    if (!currentRoomCode) return actorSlotKey(turn, options);
    const own = v3411OwnAssignment();
    if (own?.team === turn?.team && v3411TurnKeys(turn).includes(own.slotKey)) return own.slotKey;
    return actorSlotKey(turn, options);
  }

  acceptRequest = function acceptRequestV3411(id, options = {}) {
    const request = flow.assist.requests[id];
    const turn = currentTurn();
    if (!request || !turn || request.status !== "pending" || request.team !== turn.team) return;
    if (!options.botSystem && !v3411OwnsCurrentTurn()) return;
    const character = characters.find(item => item.name === request.characterName);
    if (!character || !baseIsCharacterAvailable(character, turn)) {
      delete flow.assist.requests[id];
      writeAssist();
    ...
    const sourceKey = v3411CurrentActorSlotKey(turn, options);
    if (!sourceKey || sourceKey === request.slotKey) return;
    // Correct rule: actor swaps with the exact teammate who requested the pick.
    swapSlots(turn.team, sourceKey, request.slotKey);
    delete flow.assist.requests[id];
    writeAssist();
    state.selected = character;
  ...
  acceptProposal = function acceptProposalV3411(id, options = {}) {
    const proposal = flow.assist.proposals[id];
    const turn = currentTurn();
    if (!proposal || !turn || proposal.status !== "pending") return;
    const isRecipient = currentRoomCode && proposal.targetClientId === onlineClientId();
    ...
    // Correct rule: initiator swaps with the exact teammate he selected.
    swapSlots(proposal.team, proposal.sourceSlotKey, proposal.targetSlotKey);
    delete flow.assist.proposals[id];
    writeAssist();
    if (currentRoomCode && currentRole !== "host" && !options.botSystem) {
  ...
  const baseRenderAssistUiV3411 = renderAssistUi;
  renderAssistUi = function renderAssistUiV3411() {
    cleanExpiredAssist();
    updateRequestButton();
    v3411BindDelegatableSlots();
    renderRequestBubbles();
  ...
  let v3411WatchdogKey = "";
  function v3411HostTurnWatchdog() {
    ...
    if (!state.turnDeadlineAt || onlineNow() < Number(state.turnDeadlineAt) + 900) return;
    const key = `${currentRoomCode}:${state.draftSessionId}:${state.turnIndex}:${state.turnDeadlineAt}`;
    if (v3411WatchdogKey === key) return;
    v3411WatchdogKey = key;
    state.selected = null;
    state.preselectLocked = false;
    try { autoResolveTurn({ onlineSystem: true }); }
    catch (error) { console.warn("RPmods watchdog no pudo resolver el turno.", error); }
  }

  scheduleTestingBotTurn = function scheduleTestingBotTurnV3411() {
    try { clearTestingBotTurnTimer(); } catch (_) {}
    if (!currentRoomCode || currentRole !== "host" || flow.phase !== "draft" || !state.draftActive || state.locked || state.roulette.active) return;
    const turn = currentTurn();
    if (!turn) return;
    ...
    activeBotSlots.forEach(({ slotKey }, index) => {
      const key = `botTurnV3411:${state.draftSessionId}:${state.turnIndex}:${slotKey}`;
      if (flow.botKeys[key]) return;
      flow.botKeys[key] = true;
      const turnIndexSnapshot = state.turnIndex;
      const sessionSnapshot = state.draftSessionId;
      botTimeout(() => {
        ...
        if (pendingRequest) {
          if (Math.random() < 0.82) acceptRequest(pendingRequest.id, { botSystem: true, botSlotKey: slotKey });
          else rejectRequest(pendingRequest.id, { botSystem: true });
          delete flow.botKeys[key];
          return;
        }
        ...
        state.selected = character;
        confirmTurn(true, { onlineSystem: true, botSlotKey: slotKey });
      }, randomDelay(900, 1550) + index * 340);
    });
  };

  if (!window.__rpmodsV3411Interval) {
    window.__rpmodsV3411Interval = window.setInterval(() => {
      if (flow.phase !== "draft" || !state.draftActive) return;
      renderAssistUi();
      v3411HostTurnWatchdog();
    }, 260);
  }
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — definitive turn owner resolver and slot action recovery
   * ---------------------------------------------------------------- */
  function v3412Norm(value) {
    return String(value || "").trim().toLowerCase();
  }

  function v3412TurnKeys(turn = currentTurn()) {
    return (turn?.slotKeys || [turn?.slotKey]).filter(Boolean);
  }

  function v3412AssignmentByClientOrName() {
    if (!currentRoomCode || !isAdvancedDraftConfig()) return null;
    const byClient = advancedAssignmentForClient(onlineClientId(), state.onlineSlots);
    if (byClient?.team && byClient?.slotKey) return byClient;

    // Host-created rooms can assign the host by display name instead of participant role.
    const myName = v3412Norm(currentOnlinePlayerName);
    if (!myName) return null;
    for (const team of ["A", "B"]) {
      for (const slotKey of advancedSlotsForTeamSize(activeTeamSize())) {
        const slot = state.onlineSlots?.[team]?.[slotKey];
        if (!slot) continue;
        if (v3412Norm(slot.name) === myName) return { team, slotKey, slot };
      }
    }
    return null;
  }

  function v3412IsOwnTurn(turn = currentTurn()) {
    if (flow.phase !== "draft" || !state.draftActive || !turn) return false;
    if (state.turnStartedAt && onlineNow() < Number(state.turnStartedAt)) return false;
    if (!currentRoomCode) return true;

    if (isAdvancedDraftConfig()) {
      const own = v3412AssignmentByClientOrName();
      if (!own || own.team !== turn.team) return false;
      if (!v3412TurnKeys(turn).includes(own.slotKey)) return false;
      if (turn.type === "pick" && turn.simultaneous && simultaneousRecord(turn)[own.slotKey]) return false;
      return true;
    }

    return currentRole === "player" && currentOnlineTeamLetter() === turn.team;
  }

  function v3412ActiveSlotIsBot(turn = currentTurn()) {
    if (!currentRoomCode || !turn || !isAdvancedDraftConfig()) return false;
    return v3412TurnKeys(turn).some(slotKey => isTestingBotParticipant(state.onlineSlots?.[turn.team]?.[slotKey]));
  }

  function v3412CanShowSelector() {
    if (flow.phase !== "draft" || !state.draftActive || state.locked || state.roulette.active) return false;
    return Boolean(v3412IsOwnTurn() || canRequest() || flow.assistTarget);
  }

  canControlCurrentTurn = function canControlCurrentTurnV3412() {
    return v3412IsOwnTurn();
  };

  canDelegate = function canDelegateV3412() {
    const turn = currentTurn();
    const own = v3412AssignmentByClientOrName();
    if (!v3412IsOwnTurn(turn) || !isAdvancedDraftConfig() || !own) return false;
    if (!turn || turn.type !== "pick" || own.team !== turn.team) return false;
    const mode = currentDraftConfig().delegationMode;
    if (mode === "none") return false;
    if (mode === "all_members") return true;
    if (mode === "captain_only") return own.slotKey === "captain";
  ...
  shouldShowSelectorV348 = v3412CanShowSelector;

  syncSelectorVisibilityV348 = function syncSelectorVisibilityV3412() {
    const show = v3412CanShowSelector();
    document.body.classList.toggle("rp348-selector-hidden", !show);
    document.body.classList.toggle("rp3410-selector-force-visible", show);
    document.body.classList.toggle("rp3411-selector-owned", show);
    document.body.classList.toggle("rp3412-selector-visible", show);

    const nodes = [characterGrid, document.getElementById("character-grid"), document.querySelector(".character-grid")].filter(Boolean);
    nodes.forEach(node => {
      node.setAttribute("aria-hidden", show ? "false" : "true");
      node.style.opacity = show ? "1" : "0";
      node.style.pointerEvents = show ? "auto" : "none";
      node.style.transform = show ? "none" : "translateY(8px) scale(.985)";
      node.style.filter = show ? "none" : "saturate(.65) brightness(.8)";
      node.style.visibility = show ? "visible" : "hidden";
      if (show) node.style.removeProperty("display");
    });

    document.querySelectorAll(".character-carousel, .faction-tabs").forEach(node => {
      node.style.visibility = show ? "visible" : "hidden";
      node.style.pointerEvents = show ? "auto" : "none";
      if (show) node.style.removeProperty("display");
    });
  ...
  function v3412TeamFromElement(element) {
    if (element?.closest?.("#team-a-slots") || element?.classList?.contains("team-a")) return "A";
    if (element?.closest?.("#team-b-slots") || element?.classList?.contains("team-b")) return "B";
    return "";
  }

  function v3412SlotKeyFromElement(element) {
    const raw = element?.dataset?.slotIndex;
    const index = Number.isFinite(Number(raw)) ? Number(raw) : Array.from(element?.parentElement?.children || []).indexOf(element);
    return advancedSlotsForTeamSize(activeTeamSize())[index] || null;
  }

  function v3412CanDelegateTo(team, slotKey) {
    const turn = currentTurn();
    if (!canDelegate() || !turn || turn.type !== "pick" || team !== turn.team || !slotKey) return false;
    if (v3412TurnKeys(turn).includes(slotKey)) return false;
    const slot = state.onlineSlots?.[team]?.[slotKey];
    if (!slot?.clientId) return false;
    return !state.picks?.[team]?.[advancedSlotIndex(slotKey)];
  }

  function v3412BindSlots() {
    document.querySelectorAll("#team-a-slots .player-slot, #team-b-slots .player-slot, #team-a-slots > *, #team-b-slots > *").forEach(element => {
      const team = v3412TeamFromElement(element);
      const slotKey = v3412SlotKeyFromElement(element);
      const valid = v3412CanDelegateTo(team, slotKey);
      element.classList.toggle("rp346-delegatable-slot", valid);
      element.classList.toggle("rp3412-delegatable-slot", valid);
      if (valid) {
        element.dataset.rp346TargetTeam = team;
        element.dataset.rp346TargetSlot = slotKey;
        element.title = "Elegir laminante para este compañero";
      } else {
        delete element.dataset.rp346TargetTeam;
        delete element.dataset.rp346TargetSlot;
        if (element.title === "Elegir laminante para este compañero") element.removeAttribute("title");
      }
    });
  }

  bindSlotMenus = function bindSlotMenusV3412() {
    v3412BindSlots();
  };

  document.addEventListener("pointerdown", event => {
    const element = event.target?.closest?.("#team-a-slots .player-slot, #team-b-slots .player-slot, #team-a-slots > *, #team-b-slots > *");
    if (!element) return;
    const team = v3412TeamFromElement(element);
    const slotKey = v3412SlotKeyFromElement(element);
    if (!v3412CanDelegateTo(team, slotKey)) return;
    const slot = state.onlineSlots?.[team]?.[slotKey];
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
  ...
  const baseRenderAssistUiV3412 = renderAssistUi;
  renderAssistUi = function renderAssistUiV3412() {
    cleanExpiredAssist();
    updateRequestButton();
    v3412BindSlots();
    renderRequestBubbles();
  ...
  const baseStartTurnV3412 = startTurn;
  startTurn = function startTurnV3412(options = {}) {
    baseStartTurnV3412(options);
    scheduleDraftTimeout(v3412RefreshUi, 40);
    scheduleDraftTimeout(v3412RefreshUi, 180);
    scheduleDraftTimeout(v3412RefreshUi, 420);
  };

  if (!window.__rpmodsV3412UiInterval) {
    window.__rpmodsV3412UiInterval = window.setInterval(v3412RefreshUi, 160);
  }

  // Final bot policy: bots only resolve their own active slots and pending requests. They do not create proposals.
  scheduleTestingBotTurn = function scheduleTestingBotTurnV3412() {
    try { clearTestingBotTurnTimer(); } catch (_) {}
    if (!currentRoomCode || currentRole !== "host" || flow.phase !== "draft" || !state.draftActive || state.locked || state.roulette.active) return;
    const turn = currentTurn();
    if (!turn) return;
    ...
    const botActors = botSlotsForTurn(turn).filter(item => {
      if (turn.type !== "pick") return true;
      return !turn.simultaneous || !simultaneousRecord(turn)[item.slotKey];
    });

    botActors.forEach(({ slotKey }, index) => {
      const key = `botTurnV3412:${state.draftSessionId}:${state.turnIndex}:${slotKey}`;
      if (flow.botKeys[key]) return;
      flow.botKeys[key] = true;
      const turnIndexSnapshot = state.turnIndex;
      const sessionSnapshot = state.draftSessionId;
      botTimeout(() => {
        ...
        const pending = Object.values(flow.assist.requests || {}).find(item =>
          item?.status === "pending" &&
          item.team === turn.team &&
          item.turnIndex === state.turnIndex &&
          item.draftSessionId === state.draftSessionId &&
        ...
        if (pending) {
          if (Math.random() < 0.86) acceptRequest(pending.id, { botSystem: true, botSlotKey: slotKey });
          else rejectRequest(pending.id, { botSystem: true });
          delete flow.botKeys[key];
          return;
        }
        ...
        state.selected = character;
        confirmTurn(true, { onlineSystem: true, botSlotKey: slotKey });
      }, randomDelay(760, 1380) + index * 300);
    });
  };

  let v3412WatchdogKey = "";
  window.setInterval(() => {
    if (!currentRoomCode || currentRole !== "host" || flow.phase !== "draft" || !state.draftActive) return;
    const turn = currentTurn();
    if (!turn || state.locked || state.roulette.active) return;
    scheduleTestingBotTurn();
    if (!state.turnDeadlineAt || onlineNow() < Number(state.turnDeadlineAt) + 1200) return;
    const key = `${currentRoomCode}:${state.draftSessionId}:${state.turnIndex}:${state.turnDeadlineAt}`;
    if (v3412WatchdogKey === key) return;
    v3412WatchdogKey = key;
    if (v3412ActiveSlotIsBot(turn)) {
      const character = chooseBotCharacter(turn.team);
      if (character) {
        state.selected = character;
        confirmTurn(true, { onlineSystem: true, botSlotKey: v3412TurnKeys(turn).find(slotKey => isTestingBotParticipant(state.onlineSlots?.[turn.team]?.[slotKey])) });
      }
    }
  }, 550);
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — post-ban simultaneous anti-stall and safe bot finalizer
   * ---------------------------------------------------------------- */
  function v3413TurnKeys(turn = currentTurn()) {
    return (turn?.slotKeys || [turn?.slotKey]).filter(Boolean);
  }

  function v3413TurnKey(turn = currentTurn()) {
    return `${currentRoomCode || "local"}:${state.draftSessionId}:${state.turnIndex}:${turn?.team || "-"}:${turn?.type || "-"}:${v3413TurnKeys(turn).join("+")}`;
  }

  function v3413PickValidCharacter(turn = currentTurn()) {
    const valid = characters.filter(character => isCharacterAvailable(character, turn));
    if (!valid.length) return null;
    if (turn?.type === "pick") {
  ...
  async function v3413FillPendingSimultaneous(turn, reason = "watchdog") {
    if (!turn?.simultaneous || !currentRoomCode || currentRole !== "host") return false;
    const record = simultaneousRecord(turn);
    const pending = v3413TurnKeys(turn).filter(slotKey => !record[slotKey]);
    if (!pending.length) return false;

    const originalLocked = state.locked;
    state.locked = false;
    let changed = false;
    for (const slotKey of pending) {
      if (state.turnIndex >= activeTurnCount()) break;
      if (currentTurn()?.groupId !== turn.groupId || currentTurn()?.team !== turn.team) break;
      if (simultaneousRecord(turn)[slotKey]) continue;
      const character = v3413PickValidCharacter(turn);
      ...
      try {
        ...
        console.warn("RPmods v3.4.18 no pudo completar slot simultáneo.", error);
      }
      await delay(120);
    }
    state.locked = originalLocked && !changed ? originalLocked : false;
    return changed;
  }

  function v3413ResolveSingleTurn(turn, reason = "watchdog") {
    if (!turn || !currentRoomCode || currentRole !== "host") return false;
    const character = v3413PickValidCharacter(turn);
    if (!character) return false;
    state.locked = false;
    state.selected = character;
    try {
      ...
      console.warn("RPmods v3.4.18 no pudo resolver turno simple.", error);
      return false;
    }
  }

  const baseScheduleTestingBotTurnV3413 = scheduleTestingBotTurn;
  scheduleTestingBotTurn = function scheduleTestingBotTurnV3413() {
    if (!currentRoomCode || currentRole !== "host" || flow.phase !== "draft" || !state.draftActive || state.roulette.active) return;
    const turn = currentTurn();
    if (!turn) return;

    // Keep previous working bot behaviour for normal bot-owned slots.
    try { baseScheduleTestingBotTurnV3413(); } catch (error) { console.warn("RPmods bot scheduler base falló.", error); }

    // Additional safety: after bans, simultaneous remaining-pick turns may include inactive humans.
    // Do not touch before timeout; after timeout the host fills only missing slots.
    if (!state.turnDeadlineAt) return;
    const lateBy = onlineNow() - Number(state.turnDeadlineAt || 0);
    if (lateBy < 900) return;
    const key = `v3413:schedule:${v3413TurnKey(turn)}:${Math.floor(lateBy / 1200)}`;
    if (flow.botKeys[key]) return;
    flow.botKeys[key] = true;
    if (turn.simultaneous) {
      void v3413FillPendingSimultaneous(turn, "bot-safety");
    } else if (!state.locked) {
      v3413ResolveSingleTurn(turn, "bot-safety");
    } else {
      state.locked = false;
      v3413ResolveSingleTurn(turn, "bot-safety-unlock");
    }
  };

  let v3413WatchdogKey = "";
  if (!window.__rpmodsV3413Watchdog) {
    window.__rpmodsV3413Watchdog = window.setInterval(() => {
      if (!currentRoomCode || currentRole !== "host" || flow.phase !== "draft" || !state.draftActive || state.roulette.active) return;
      const turn = currentTurn();
      if (!turn || !state.turnDeadlineAt) return;
      const lateBy = onlineNow() - Number(state.turnDeadlineAt || 0);
      if (lateBy < 1250) {
        try { scheduleTestingBotTurn(); } catch (_) {}
        return;
      }
      const key = `${v3413TurnKey(turn)}:${Math.floor(lateBy / 1800)}`;
      if (v3413WatchdogKey === key) return;
      v3413WatchdogKey = key;

      // If the base auto-random selected a character but did not commit it, release the lock and commit/fill.
      if (turn.simultaneous) {
        void v3413FillPendingSimultaneous(turn, "deadline-watchdog");
      } else {
        v3413ResolveSingleTurn(turn, "deadline-watchdog");
      }
    }, 420);
  }

  // If a simultaneous group already has all records but did not advance, force finalization.
  if (!window.__rpmodsV3413FinalizeGuard) {
    window.__rpmodsV3413FinalizeGuard = window.setInterval(() => {
      if (!currentRoomCode || currentRole !== "host" || flow.phase !== "draft" || !state.draftActive) return;
      const turn = currentTurn();
      if (!turn?.simultaneous) return;
      const record = simultaneousRecord(turn);
      const complete = v3413TurnKeys(turn).length && v3413TurnKeys(turn).every(slotKey => Boolean(record[slotKey]));
      if (!complete) return;
      const key = `v3413:finalize:${v3413TurnKey(turn)}`;
      if (flow.botKeys[key]) return;
      flow.botKeys[key] = true;
      state.locked = false;
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — definitive simultaneous group finalizer
   * ---------------------------------------------------------------- */
  ...
  function v3414PatchSimultaneous(turn = currentTurn()) {
    ...
    try {
      ...
      console.warn("RPmods v3.4.18 no pudo sincronizar simultáneo.", error);
    }
  }
  ...
  function v3414FinalizeGroupHard(turn = currentTurn(), reason = "hard-finalize") {
    ...
    for (const slotKey of v3414TurnSlotKeys(turn)) {
      ...
      if (!character) {
        console.warn("RPmods v3.4.18 no encontró personaje para slot simultáneo", slotKey, reason);
        continue;
      }
      record[slotKey] = character.name;
    }
    ...
    try {
      ...
      console.warn("RPmods v3.4.18 finalizeSimultaneousGroup falló; usando avance manual.", error);
  ...
  function v3414MaybeRescueTurn(reason = "watchdog") {
    ...
    // This is the critical case from v3.4.18: one slot says "esperando al compañero".
  ...
  scheduleTestingBotTurn = function scheduleTestingBotTurnV3414() {
    try { baseScheduleTestingBotTurnV3414(); } catch (error) { console.warn("RPmods v3.4.18 base bot scheduler falló.", error); }
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — sequential remaining picks, no double-pick groups
   * ---------------------------------------------------------------- */
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — bot preselection delay
   * ---------------------------------------------------------------- */
  ...
  /* ------------------------------------------------------------------
   * v3.4.18 — PEDIR separated request layer
   * ---------------------------------------------------------------- */
  function requestCharacterByNameV3418(name) {
    return characters.find(character => character.name === name) || null;
  }

  function ensureRequestPanelV3418() {
    let panel = document.getElementById("rp3418-request-panel");
    if (panel) return panel;
    panel = document.createElement("section");
    panel.id = "rp3418-request-panel";
    panel.className = "rp3418-request-panel hidden";
    panel.innerHTML = `
      <div class="rp3418-request-panel-copy">
        <span>PEDIR LAMINANTE</span>
        <strong>Elige el laminante que quieres solicitar</strong>
        <p>Esta acción no modifica la preselección ni el turno actual del capitán/subcapitán.</p>
      </div>
      <button type="button" class="rp3418-request-cancel">CANCELAR</button>
    `;
    document.body.appendChild(panel);
    panel.querySelector(".rp3418-request-cancel")?.addEventListener("click", event => {
      event.preventDefault();
      event.stopPropagation();
      clearRequestPickModeV3417();
      showAppNotice("Pedido cancelado.", { type: "info", duration: 2400 });
    });
    return panel;
  }

  function renderRequestPanelV3418() {
    const panel = ensureRequestPanelV3418();
    const active = ownRequestModeActiveV3417();
    panel.classList.toggle("hidden", !active);
    document.body.classList.toggle("rp3418-request-layer-active", active);
  }

  // v3.4.18: PEDIR is a separated layer. It never uses CONFIRMAR and never reads
  // the captain/subcaptain current preselection.
  document.addEventListener("click", event => {
    if (!ownRequestModeActiveV3417()) return;
    const card = event.target?.closest?.("#character-grid .character-card[data-name]");
    if (!card) return;
    ...
    const character = requestCharacterByNameV3418(card.dataset.name);
    if (!character || !baseIsCharacterAvailable(character, currentTurn())) {
      showAppNotice("Ese laminante no está disponible para pedir.", { type: "warning", duration: 3200 });
      return;
    }

    const requestedName = character.name;
    flow.requestPicking = null;
    document.body.classList.remove("rp3417-request-picking", "rp3418-request-layer-active");

    // Do not preserve or push state.selected. This prevents PEDIR from affecting
    // the active player's preselection.
    state.selected = null;
    state.preselectLocked = false;

    createRequest(character);
    showAppNotice(`Solicitud enviada: ${requestedName}.`, { type: "success", duration: 3600 });
    renderRequestPanelV3418();
    renderAssistUi();
  }, true);

  const baseStartRequestPickModeV3418 = startRequestPickModeV3417;
  startRequestPickModeV3417 = function startRequestPickModeV3418() {
    if (currentRole === "host" && !ownAssignment()) return;
    baseStartRequestPickModeV3418();
    state.selected = null;
    state.preselectLocked = false;
    renderRequestPanelV3418();
  };

  const baseClearRequestPickModeV3418 = clearRequestPickModeV3417;
  clearRequestPickModeV3417 = function clearRequestPickModeV3418(options = {}) {
    baseClearRequestPickModeV3418({ ...options, keepSelection: false });
    renderRequestPanelV3418();
  };

  const baseRenderAssistUiV3418 = renderAssistUi;
  renderAssistUi = function renderAssistUiV3418() {
    baseRenderAssistUiV3418();
    renderRequestPanelV3418();
  };

  const baseUpdateRequestButtonV3418 = updateRequestButton;
  updateRequestButton = function updateRequestButtonV3418() {
    baseUpdateRequestButtonV3418();
    const button = ensureRandomSelectionButton();
    if (!button) return;

    const spectator = currentRoomCode && !ownAssignment();
    if (spectator) {
      button.dataset.rp346RequestMode = "0";
      button.removeAttribute("data-rp3417-request-picking");
      if (button.textContent === "PEDIR" || button.textContent === "CANCELAR PEDIDO") button.classList.add("hidden");
      return;
    }

    if (canRequest()) {
      button.classList.remove("hidden");
      button.textContent = ownRequestModeActiveV3417() ? "CANCELAR PEDIDO" : "PEDIR";
      button.title = ownRequestModeActiveV3417()
        ? "Cancelar modo PEDIR"
        : "Activa un selector separado para pedir un laminante";
      button.setAttribute("aria-label", button.title);
      button.dataset.rp346RequestMode = "1";
      button.dataset.rp3417RequestPicking = ownRequestModeActiveV3417() ? "1" : "0";
    }
  };

  // Replace request button behavior again: first click only activates the request layer.
  document.addEventListener("click", event => {
    const requestButton = event.target?.closest?.("#random-selection-action[data-rp346-request-mode='1']");
    if (!requestButton) return;
    ...
    if (!canRequest() || (currentRoomCode && !ownAssignment())) return;
  ...
  /* ------------------------------------------------------------------
   * v3.4.35 — stability guard for offline/online draft commits
   * ---------------------------------------------------------------- */
  ...
  /* ------------------------------------------------------------------
   * v3.4.35 — deterministic commit path for manual and timeout actions
   * ---------------------------------------------------------------- */
  ...
  /* ------------------------------------------------------------------
   * v3.4.35 — safer timeout and direct commit path for pick/ban
   * ---------------------------------------------------------------- */
